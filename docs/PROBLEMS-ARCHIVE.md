...
## RESOLVED 2026-08-28: Nuclei scan audit - 18 template-ids, 8 fixed, 2 refuted, 4 assessed and left alone

`bin/run_nuclei_scan.sh --all-tiers` against staging (10,689 templates x 4 auth tiers -
unauthenticated, a restricted-scope API key, a full-scope API key, and a real signed-in session)
came back with 86 raw hits collapsing to 18 unique template-ids, every one `info` severity. Each
was traced to a specific line of this codebase (or refuted with one) rather than taken at the
scanner's word.

### Fixed

**`missing-sri`: 12 of the 16 script/style entries in `VENDOR_ASSETS` had no integrity hash.**
`toastr_js`, `htmx_js` and `jquery_js` were already pinned (see the HTMX entry elsewhere in this
file); the Leaflet family (8 entries), `toastr_css`, `chartjs_js`, `sortable_js` and
`fontawesome_css` were not. Fixed by fetching each CDN URL's actual bytes and computing its sha384
the same way the HTMX fix did, rather than guessing - `dashboard/services/core/vendor_assets.py`.
`test_vendor_assets.py::test_every_script_and_style_pins_an_integrity_hash` now asserts every
non-image entry in the table carries one, so the next asset added without a hash fails there
instead of waiting for a scan. The two Google Fonts CSS URLs the scan also flagged
(`fonts.googleapis.com/css2?...`, `fonts.googleapis.com/icon?...`) are not fixed and are not going
to be - see Refuted below.

**`http-missing-security-headers`: `permissions-policy`, `cross-origin-resource-policy` and
`x-permitted-cross-domain-policies` were genuinely absent.** Django's `SecurityMiddleware` has a
setting for `X-Content-Type-Options`, `Referrer-Policy` and (since 4.0) `Cross-Origin-Opener-Policy`
- all three were already at their secure defaults and were never actually missing, confirmed by
reading `SecurityMiddleware.process_response` directly rather than trusting the scanner's `info`
line - but it has no setting at all for the other three. Added a `SecurityHeadersMiddleware`
(`dashboard/middleware.py:25`) installed directly below `SecurityMiddleware`
(`settings/base.py` `MIDDLEWARE`), reading `settings.PERMISSIONS_POLICY` /
`settings.CROSS_ORIGIN_RESOURCE_POLICY` / `settings.X_PERMITTED_CROSS_DOMAIN_POLICIES`
(`settings/base.py:595-606`). Permissions-Policy denies `camera`/`microphone`/`payment`/`usb`
outright and scopes `geolocation`/`clipboard-write` to `self` - the only two the frontend actually
calls (`safety-live-location.ts`, `map-context-menu.ts`). Cross-Origin-Resource-Policy is
`same-site`, not `same-origin`, because REData and the staging/dev slots are separate origins under
the same parent domain. `cross-origin-embedder-policy` is the fourth thing this template flagged and
is not fixed - see Assessed below.

**`exposed-file-upload-form`: real markup, not a real exposure, fixed anyway.** The scan matched
`<input type="file">` on the signed-out landing page. That input is `themes/base.html`'s
"Attach a Photo" comment dialog (`cip-file-input`), rendered unconditionally into every page
including the ones an anonymous visitor gets - `CommentImagePickerView` and every comment-POST
endpoint behind it are `LoginRequiredMixin` (`controllers/comments.py`), so nobody signed out could
ever have opened or submitted it. The scanner can't see that; it just greps the body. Fixed anyway,
because the actual reason it doesn't matter is also the reason not to ship it: gated the whole
dialog behind `{% if request.user.is_authenticated %}` (`themes/base.html:373`). The inline script
that wires it up already guarded `if (!dialog) return;` right after `getElementById`, so a
signed-out response with the block simply absent needed no JS change.
`test_signed_out_homepage.py::test_signed_out_visitors_get_no_file_upload_markup` and
`SignedInHomepageTests::test_signed_in_visitors_still_get_the_photo_dialog` cover both directions.

**Found while fixing `missing-sri`, not itself a scan finding: `map-annotations.ts` had its own,
unmigrated copy of the exact bug `vendor_assets.py` exists to prevent.** Its marker icon was
hardcoded to `cdnjs.cloudflare.com/ajax/libs/leaflet/1.7.1/images/marker-icon.png` - Leaflet 1.7.1,
from cdnjs - while the rest of the app is on 1.9.4 from unpkg (`map/index.html`'s inline script
already resolved the same icon correctly, through `{% vendor_asset_source %}`). `vendor_assets.py`'s
own module docstring names this precise failure mode ("Leaflet's marker images came to be served
from a different release of Leaflet than the library using them") as the reason the table exists,
but the table only replaced references inside `templates/**/*.html`
(`NoRawCdnUrlsInTemplatesTests` in `test_vendor_assets.py` only greps `.html`); `map-annotations.ts`
is a compiled TS entry point outside that tree and was missed. Fixed by threading
`data-marker-icon-url`/`data-marker-shadow-url` through `#map-annotations-config`
(`location/index.html`, `location/wiki.html`) - this bundle's own established pattern for getting
server-rendered config into it, documented at the top of the file - instead of hardcoding a URL.
Also rebuilt the compiled bundle (`bun run build`); `dashboard/frontend/static/dashboard/js/` is the
real build output, `src/urbanlens/frontend/static/` is a separately-committed `collectstatic` copy
that `bin/init.py` regenerates from the former at every container start (`docker-compose.yml:168`)
and was several days stale even before this - not fixed here, flagged for Jess, since deciding
whether that copy should be committed at all is a separate question from this scan.

### Refuted, and worth recording

**`php-errors` matched on a JavaScript comment, not an error.** `extracted-results` was
`"uncaught exception"`. There is no PHP anywhere in this codebase (`find . -name "*.php"` is empty)
- what the template's generic PHP-error-signature regex actually caught is
`themes/base.html:1175`, `// Array.forEach aborts entirely on an uncaught exception, and one...`, a
plain code comment inside an inline `<script>` block that ships to the browser verbatim (this
project does not strip comments from inline script). Coincidence, not a defect.

**`cookies-without-httponly` matched `csrftoken`, which is deliberate.**
`test_security_headers.py::test_session_cookie_is_http_only`'s own docstring already says why: "The
CSRF cookie deliberately is not [HttpOnly] - the frontend reads it." `SESSION_COOKIE_HTTPONLY` is
`True` (Django's default, unoverridden) and was never the one flagged.

### Assessed and left alone

- **`missing-cookie-samesite-strict`**: both cookies ship `SameSite=Lax` (Django's default, also
  unoverridden), not `Strict`. `Strict` would stop the session cookie being sent on the *first* hit
  of a shared pin/wiki link opened from outside the site (email, Discord, a chat app) while already
  signed in elsewhere - directly against this app's own link-sharing feature - for marginal benefit
  over `Lax`: CSRF is enforced by Django's token mechanism regardless of `SameSite`, and `Lax`
  already blocks the cross-site POST vector this class of check exists for. Left at `Lax`.
- **`cross-origin-embedder-policy`**, the fourth header `http-missing-security-headers` flagged:
  `require-corp` would block every third-party resource on this site that does not send its own
  CORP/CORS header - the Street View iframe, the OSM/ArcGIS/OpenTopoMap tile hosts, Gravatar, any
  operator-pasted map-overlay image - none of which this project controls. Same posture CSP itself
  takes (report-only until proven clean): an `info`-severity finding is not worth a same-day risk of
  a broken map.
- **`wildcard-dns-detect` / `dns-saas-service-detection`**: DNS/hosting facts
  (`*.staging.urbanlens.org` resolves to the app's own IP; the domain's DNS is hosted by a named
  SaaS provider), not application code - nothing here to change from inside this repository.
- **`options-method`, `addeventlistener-detect`, `form-detection`, `tech-detect`,
  `fingerprinthub-web-fingerprints`, `waf-detect`, `openresty-detect`, `ssl-dns-names`,
  `ssl-issuer`, `tls-version`**: pure reconnaissance templates. `options-method`'s own
  `extracted-results` was `"GET, HEAD, OPTIONS"` - no state-changing verb exposed beyond what every
  page already accepts. The rest report facts (a form exists, TLS 1.3 is in use, nginx is behind a
  WAF) rather than anything wrong.
