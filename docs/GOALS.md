...
This is the authoritative, human-owned statement of product intent. It is intentionally short — a decision record, not a spec. Jess edits this file directly; **It as read-only to agents**. Where an
...
- A pin and everything inside it is private by default. There is no setting that makes a pin
  visible or searchable to another user. Full stop.
...
- Every REST endpoint and query must enforce this **by construction** — it must be
  structurally impossible for a bug to leak a private pin's data through some other surface, not just conventionally discouraged.
...
- Store gameplay-derived data in a schema general enough to support future/different analysis
  approaches, not just whatever ML pipeline is in use today. We want historical data to use when we
  design a better architecture to use it.
...
- Both should be independently sufficient. A user should be able to find anything on the site
  they have access to by browsing alone (Memories, recents, saved filters, lists) with no search — and find
  anything they can access by searching alone with no browsing. Audit for gaps against that bar.
- Every map on the site has the same controls, layout, and behavior (where they apply) — no page should require
...
- "Public pins" are pins that can be added or suggested to users across the entire site, because they
  have been deemed safe and well known. This allows new users to have "something" on their map after registering. The bar for this is incredibly high.
...
- UrbanLens is open source. A self hosted deployment (no Kubernetes) must work fine without
...
- Hypothesis property-based tests should be used when possible. Redundant coverage via different strategies is a feature.
- Every page has (at an absolute minimum) one unit test and at least one integration test proving it loads with the expected content; every feature eventually gets the same coverage too.
