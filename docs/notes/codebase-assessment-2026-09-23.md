...
`id: N28` · `status: current` · `updated: 2026-09-24`
...
This is not a file-complete audit. Batches 1–35 were re-read in the source before they were written. Later batches were still finding defects, mostly further instances of shapes already seen: a queryset loaded and then capped in Python, an upstream call on the request, `safely_enqueue_task` whose return value is ignored, and rows that are inserted and never deleted.

A later pass should start here, not by re-reading the map:

- `plugins/` was sampled for a user-supplied URL fetch and not read plugin by plugin.
- Frontend TypeScript and SCSS were not walked. The map shell and three classic entries (`mention-autocomplete.ts`, `popup-dismiss.ts`, `organize-filter-engine.ts`) were. The three entries did not show a missing install-once guard. Other entries were not opened.
- Templates were spot-checked for `|safe` (`render_article`, pin notes, `extra_html`, `safe_json_for_script`). They were not read template by template.
- `external_api/` was sampled (pagination, the memories timeline, a few bulk views that copy a controller). It was not read view by view.
- Migrations, the migration graph, and the test suite as a specification were not reviewed. No pytest, load test, or browser session was run. Comments that say a cost was measured were not re-measured.
- `docs/PROBLEMS.md` and `docs/archive/` were searched for claims about to be repeated, not read end to end. A fix recorded as done may have regressed.
- Encryption was checked for decrypt-on-delete (batch 12) and for fields that decrypt when a row is loaded (`Image.original_filename`, OAuth tokens, invitation text). `rotate_field_encryption` and a full key-rotation pass were not done.
- The Celery beat schedule was not held up against every table that grows. Several "nothing deletes these rows" findings are in batches 27–33. Others may remain.
- `settings/app.py` was not read field by field. `DEBUG` and `csp_enforce` defaults are still only a lead under "Not re-opened". `P56` is COEP, not CSP.
- Other `requests.get` / `requests.post` call sites were not enumerated. Flickr and Immich panels, photo proxies, Immich thumbnails, and Street View metadata are still leads: several neighbours are wrapped in `call_with_deadline` or `UpstreamSlots`, and `P113` parked four. Diff before filing.
- `GoogleMapsGateway.import_pins_streaming` is the dead path in `P20`. GPX dwell detection, location-history import, and My Activity import are only reached from it. They were not filed. If that method is called again, that cost is live.
- A process with `UL_RABBITMQ_URL` unset still falls back to Dragonfly (batch 13). Compose sets the variable. A deployment without it was not checked.

Useful greps, aimed at files not already cited above: `safely_enqueue_task(` with no use of the return value; `for ` over a queryset that later `break`s or slices in Python; `session.get` / `session.post` inside `controllers/`; `get_or_create(` on a `OneToOneField`.
