...
| 6 | Celery queue classes; move P96/P98/P2 work off the request | **queue classes built 2026-09-11** (D13: all 96 tasks classified, `celery-worker` narrowed to `-Q interactive`, `celery-worker-bulk` added, startup check `dashboard.E010`). **P96 moved to a bulk task 2026-09-13** - one import per account, stored behind a job id, polled. **P98's media walk moved to a maintenance task 2026-09-13.** P2 not started |
