...
| H63 | low | anonymous | `src/urbanlens/dashboard/controllers/index.py:23` | The landing page's first render in each worker costs ~0.57s CPU in-container (1.4s on staging) against 0.005s warm, inside `rendered_content` rather than template lookup. Under gevent that is CPU that never yields, so other in-flight requests on the worker wait. Warming via `get_template()` does NOT fix it (0.568s -> 0.528s) - the cost is the lazy `{% include %}` chain and its tag imports. | render once at boot; see N22 |
