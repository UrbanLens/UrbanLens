...
**This table records the findings as first written; it is not a status board.** Rows carry a marker
where this effort has direct evidence of their resolution, but an unmarked row means only that
nobody has written a marker on it — several were fixed by the batches listed above without the row
being touched. **P113 is the current status**, and the code beats both.
