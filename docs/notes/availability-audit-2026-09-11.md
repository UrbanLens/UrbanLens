...
| H64 | low | own-data-only | `src/urbanlens/dashboard/tasks.py:1432` | A file the database references and the disk does not have is retried by the thumbnail backfill on every cursor wrap, forever, logging a full traceback each time. `write_image_preview` already solves this shape a few lines away by caching `UNPREVIEWABLE` against a failure TTL; the thumbnail path has no equivalent. | mark unreadable rows; see N22 |
