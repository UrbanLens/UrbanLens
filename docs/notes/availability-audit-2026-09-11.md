...
**This table records the findings as first written; it is not a status board.** Every row was
re-checked against the code on 2026-09-13 and now carries either a closure marker or `STILL OPEN`,
but a marker is a claim about that date and nothing keeps it true. **The work list is the board in
P113**, and the code beats both.

| ref | severity | reachable by | file | what it is | smallest fix |
|---|---|---|---|---|---|
| H01 — fixed 2026-09-12 (queue, single-flight, grid key, weighted hits) | critical | any-authenticated | `src/urbanlens/dashboard/tasks.py:2231` | The Immich "Scan your library" button enqueues an uncapped, un-deduplicated whole-library sweep that holds one of only four default-queue worker slots for up to 45 minutes while issuing one PostGIS query per photo. |  |
| H02 — thumbnail half already fixed when re-checked | critical | any-authenticated | `src/urbanlens/dashboard/controllers/google_photos.py:290` | The Google Photos thumbnail proxy stores a full 2048px preview of every picked photo in the shared 512MB Valkey for an hour, and the number of previews is whatever the user selected in the picker. |  |
...
| H52 — fixed 2026-09-12 (the panel pages, and the badge counts the thread) | high | own-data-only | `src/urbanlens/dashboard/services/trips/trip_comments.py:132` | The trip comments panel renders every comment and reply with no pagination, and re-runs the author-visibility relationship queries per comment - each of which loads both profiles' entire pin sets |  |
| H53 — already fixed when re-checked | high | own-data-only | `src/urbanlens/dashboard/controllers/albums.py:366` | Album detail embeds a JSON entry for every photo in the album inline in the page, while the photo grid itself is paginated at 48 |  |
| H54 — STILL OPEN as of 2026-09-13 | high | own-data-only | `docker-compose.yml:575` | One 512MB Valkey holds sessions, the Channels layer, the Django cache and the Celery broker in a single keyspace under volatile-lru, and the Celery broker's keys are the only ones with no TTL - so a user-driven task backlog is paid for by e |  |
| H55 — fixed 2026-09-13 (a sized tmpfs, so the spool cannot reach the database's disk) | high | anonymous | `src/urbanlens/config/nginx/nginx.conf:12` | nginx spools up to 200MB of every request body to disk in the container's writable layer before Django sees the request, so unauthenticated POST volume consumes the same host filesystem that holds the Postgres data directory. |  |
| H56 — STILL OPEN as of 2026-09-13 | high | any-authenticated | `package.json:16` | A request that spends >180 s in non-yielding Python/C CPU causes gunicorn to abort the whole gevent worker process, killing up to 19 other users' in-flight requests with it. |  |
| H57 — fixed 2026-09-12 (a range the index can answer) | high | any-authenticated | `src/urbanlens/dashboard/services/core/rate_limiter.py:584` | The outbound-API rate limiter's own daily-window check is a non-indexable count over a 400-day append-only log, executed on every external call while holding that service's global row lock. |  |
| H58 — fixed 2026-09-12 (assert_restorable, one query per relation) | high | own-data-only | `src/urbanlens/dashboard/services/undo/handlers/pin.py:142` | Undoing a bulk pin delete recreates the whole stashed subtree one row at a time — 5 pre-flight queries per pin, then a full `Pin.objects.create()` cascade per pin — inside a single transaction holding the undo row's lock. |  |
| H59 — fixed 2026-09-12 (moved to `http`, plus a real `nginx -t` gate) | critical | anonymous | `src/urbanlens/config/nginx/nginx.conf:16` | `limit_conn_zone` was declared in `events {}` rather than `http {}`, so nginx refused the config and never bound a port - the whole site, not a degraded one. Three tests covered the directive and passed, because each was an `assertIn` against whole-file text that cannot see which block it landed in. The config is bind-mounted, so a running nginx kept serving from what it parsed at boot and the break stayed invisible until a restart. | N22 |
| H60 — fixed 2026-09-12 (each hook checks its process) | high | n/a (boot) | `gunicorn.conf.py:1` | gunicorn reads `gunicorn.conf.py` from the working directory with no `-c`, and that directory is `/app` for every service in this image - so the Django app's config also configured `ai-inference`, applying psycogreen's *gevent* patch inside a *gthread* worker and logging an ImproperlyConfigured traceback every boot. The worker-class guard is D11's prerequisite. | N22 |
| H61 — fixed 2026-09-12 (a complete request line) | medium | n/a (observability) | `docker-compose.yml:673` | The egress proxy's `nc -z` healthcheck connected and closed without sending anything, so tinyproxy logged an ERROR every 30s - 2,880 lines a day against 200 KB x 10 of rotation, ageing real errors out of the only log that shows a blocked provider call. | N22 |
| H62 — fixed 2026-09-12 (warm the URLconf in `asgi.py`) | high | any-authenticated | `src/urbanlens/UrbanLens/asgi.py:24` | daphne never warmed the HTTP URLconf, so its first HTTP request paid a 2.26s CPU import (measured; staging showed 4176ms CPU against zero SQL). `app-ws` runs `cpus: 1` and that import holds the GIL, so it stalls the event loop serving every WebSocket on the site - after the container has reported itself healthy. | N22 |
| H63 — fixed 2026-09-12 (warm the reverse table too) | low | anonymous | `gunicorn.conf.py:131` | The URLconf warm-up read `url_patterns`, which imports the URLconf and nothing else - Django builds the *reverse* table separately and lazily, on the first `reverse()`/`{% url %}`. Every template opens with a `{% url %}`, so 0.364s of regex normalisation landed on each process's first request to any view (first `GET /`: 0.568s CPU -> 0.067s after). Found by sorting cProfile by self time; cumulative time blamed the template render and nearly bought an invasive fix. | N22 |
| H64 — fixed 2026-09-12 (media_unreadable_at, a backoff rather than a blacklist) | low | own-data-only | `src/urbanlens/dashboard/tasks.py:1432` | A file the database references and the disk does not have is retried by the thumbnail backfill on every cursor wrap, forever, logging a full traceback each time - twice, since `generate_image_marker_thumbnails` (`tasks.py:1675`) walks the same rows. `write_image_preview` already solves this shape a few lines away by caching `UNPREVIEWABLE` against a failure TTL; the thumbnail path has no equivalent. | mark unreadable rows; see N22 |
| H05 — fixed 2026-09-13 (both pickers batch both gates) | medium | any-authenticated | `src/urbanlens/dashboard/controllers/direct_messages.py:797` | The message-recipient and group-member autocompletes run a full privacy evaluation per candidate, and each evaluation reads two accounts' entire Pin tables into Python sets - on every keystroke. |  |
| H06 — fixed 2026-09-13 (all four surfaces resolve their list in one pass) | medium | any-authenticated | `src/urbanlens/dashboard/services/pins/common_pins.py:44` | Every per-row "may this viewer see that person" check loads both profiles' entire Pin tables into Python, and it is called once per photo/comment rendered. | Give trip_comments.build_comment_tree the same per-author `can_view` memoization comments.py already has, and paginate `trip.comments.filter(parent__isnull=True)` the way the pin/wiki panel  |
| H08 — closed with H26 (one cap on one view, entered twice) | medium | own-data-only | `src/urbanlens/dashboard/controllers/labels.py:1089` | Label bulk-edit/bulk-delete accept an unbounded id list, and every label saved fires a receiver that UPDATEs every pin carrying it, so one click can rewrite millions of Pin rows inside one request | Add a cap on the ids list (mirroring pin_bulk.py's _MAX_BULK_PINS / _too_many pattern) so a pathological personal label+pin collection can't turn one click into an unbounded number of large  |
| H13 — fixed 2026-09-12 (set_if_small; the select_for_update half was overstated) | medium | any-authenticated | `src/urbanlens/dashboard/controllers/basemap_tiles.py:165` | Every basemap/historical map-tile miss serializes on one global SELECT FOR UPDATE row, spends a site-wide 600/min budget, and writes the tile bytes into the shared 512MB Valkey for 7 days — so one user panning the map can blank the map and |  |
| H15 — already fixed when re-checked | medium | any-authenticated | `src/urbanlens/dashboard/controllers/memories.py:463` | The Memories feed endpoint has no pagination and no ceiling on its date window, and the page ships an "All time" button that asks for the account's entire history in one response. |  |
| H22 — size half fixed 2026-09-12 (set_if_small); key-count half is H54's Valkey split | medium | any-authenticated | `src/urbanlens/dashboard/controllers/basemap_tiles.py:172` | The REData basemap tile proxy caches raw tile bytes in the shared Valkey for seven days, one key per z/x/y a user pans over, with no size or key-count ceiling. |  |
| H23 — fixed 2026-09-13 (the superseded copy is dropped, not left to the TTL) | medium | any-authenticated | `src/urbanlens/dashboard/services/search/saved_filter_cache.py:91` | Every login and every map/toolbar load writes a full list of a profile's matching pin UUIDs into Valkey under a fingerprint-versioned key, and the superseded copy is never deleted - it lingers for a day. |  |
