...
returns without starting, so starts overlap only while the cache is down, when a
publish goes ahead unmarked and the sweep never drops); counting starts rather than queues means a sandbox queue
