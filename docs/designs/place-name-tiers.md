...
The ranking follows how specifically each kind of name identifies a *property*. A register listing is an
official survey of this property with a surveyed boundary. The Wikipedia article is curated but may cover
more or less than the parcel. A site polygon is a name someone drew around the place. A building names
...
- The HRSH campus is titled **"Hudson River State Hospital, Main Building"**, not "Hudson River State
  Hospital". That follows from the ranking Jess set out, but the listing is formally the Main Building's,
  even though its boundary covers most of the campus. **Question for Jess:** should a register listing
  whose `scope` is `structure` rank below the Wikipedia article on a multi-building parcel? That would be
  one line in `rank_key`.
