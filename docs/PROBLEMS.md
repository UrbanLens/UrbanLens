...
**Measured 2026-09-06.** One grid is 594,669 bytes / 2,498 buttons. The page renders N+1 of them,
not N - the create form has one too, which matters because it lands in the render-scaling baseline.
60 awards is ~34.6 MB and 76,189 buttons. `ICON_CATEGORIES` is in `models/labels/meta.py`, not
`services/core/icons.py`. nginx gzips the response (`config/nginx/nginx.conf:71-73`), so the cost is
Django's render and the browser's parse rather than the wire. Also: `pages/organize/index.html`
renders **13** pickers as a flat page cost - ~7.6 MB on every load, not per row - which is very
likely the residue of the twelve-second Organize Labels page `render_scaling.py`'s own docstring
cites.

**Do not fix it with `hx-get` on the placeholder, which is the obvious approach.** htmx events
bubble, and with `hx-swap="outerHTML"` the placeholder is gone by the time `htmx:afterRequest`
fires, so htmx re-dispatches it on the nearest surviving ancestor. Four templates wrap a picker in a
`<form>` carrying `hx-on::after-request` that closes a dialog and toasts success on
`event.detail.successful` - `organize_label_edit_form.html:8`, `organize_label_customize_form.html:7`,
`organize_label_create_dialog.html:5`, and `pages/map/index.html:398` (which also calls
`this.reset()`). Merely *opening* the picker would close the dialog, discard the user's edits and
toast "saved". The achievement admin itself has no `hx-on`, which is exactly why a fix validated
against the page this entry names would look fine. `hx-trigger="click once"` compounds it: htmx
spends `once` when the event fires, not when the request succeeds, so a single failed fetch leaves
that picker reading "Loading icons..." until a full page reload.

Fetching from `IconPicker.toggle` in `frontend/ts/shared/icon-picker.ts` instead emits no htmx
events and sidesteps all of it - at the cost of touching the two other copies of that contract
(`organize-icon-picker.ts`, and the hand-written one inlined at `pages/map/index.html:4830-4930`).

**A separate bug this turned up.** `clean_icon` (`services/core/icons.py:53-63`) rejects 29 of the
catalogue's own 1,249 icons - the 14 keycaps (`0️⃣`-`9️⃣`, `#️⃣`, `*️⃣`, whose base code point is
ASCII), `‼️`, `⁉️`, and 13 letter-category entries (α, π, Ω, あ, 漢, 한, א, Я...). Every write path
that routes through it - `labels.py:691`, `labels.py:814`, `services/labels/customization.py:86`,
`saved_filters.py:123`, `maps.py:829`, `pin_creation.py:291` - silently stores nothing when a user
picks one of those from the picker that offered it. `Achievement.icon` is unaffected only because it
...
## P80 — The site-admin directory's search matches on email and first name before any visibility rule, so a masked field can be confirmed by guessing

`id: P80` · `status: open` · `updated: 2026-09-06`

`SiteAdminUsersView`'s own docstring (`controllers/site_admin.py:1189-1195`) promises the opposite:
"even a site admin does not get a backdoor around a user's `contact_visibility` setting here. Email
is only shown when the viewing admin's own profile would satisfy that user's configured visibility
rule". The rendering half of that is true. The search half was never checked.

`controllers/site_admin.py:1222-1225` filters before any visibility rule is consulted:
...
Visibility is applied per row afterwards (`:1240-1241`), and only decides what the row *renders*.
So for a user with `contact_visibility = NO_ONE` and no relationship to the admin, the row correctly
shows "Hidden" - and `?q=<their address>` still returns exactly that one row, while a wrong guess
renders the empty state. One request per guess confirms or denies any address, and the same works on
`first_name` against `profile_visibility`. The two fields the page promises not to reveal are
enumerable through the box directly above them.
...
back rather than in what they say.

**Not fixed, because the two ways out are a product decision rather than a repair.**

1. **Restrict the predicate to what the viewer may see.** It cannot be done in the queryset as
   written - visibility is per-viewer and per-row - so the honest shape is to keep
   `Q(username__icontains=...)` unconditional and evaluate the email/first-name arms only against the
   pks the viewer already passes. That needs the batched visibility resolution P68 wants anyway, so
   the two are worth doing together.
2. **Accept that admin search is privileged, and rewrite the docstring.** Defensible - an admin
   troubleshooting "user says they didn't get the email" needs to find them by address - but it
   contradicts what the page currently claims about itself, and the claim is the thing users would
   rely on.

Doing neither is the one option that is clearly wrong, because the code and its own docstring
currently disagree about a privacy control.

Related: P14 (media-gate visibility), and P68's site-admin half, which is the same view.
