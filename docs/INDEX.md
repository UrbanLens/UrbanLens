...
[`archive/PROBLEMS-ARCHIVE.md`](archive/PROBLEMS-ARCHIVE.md). This index is
what is live. Grep the archive before concluding a defect
...
| D12 | accepted | 2026-09-15 | The map cache becomes an accelerator the site can lose, and labels stop being copied into every pin; built, with the delta and viewport mode deferred | [`docs/designs/map-data-contract-v11.md`](designs/map-data-contract-v11.md) |
...
| X20 | holds | 2026-09-15 | A browser label cache pays in Organize rendering and in filter and search round-trips, not in label joins: ~250 ms of server CPU an Organize view and 40-130 ms a map filter, against 1-2 ms in a browser | [`docs/notes/label-cache-and-filter-cost-measured.md`](notes/label-cache-and-filter-cost-measured.md) |
...
| I5 | actionable | 2026-09-21 | Only the Overpass overlay can move to the browser without giving up the anonymised-by-proxy guarantee `external_apis_enabled` promises | [`docs/reports/client-side-api-offload.md`](reports/client-side-api-offload.md) |
| I6 | actionable | 2026-09-21 | Prewarming at login only moves the cost; the one case worth building is the herd after a global access bump, measured at 4 extra statements per active viewer | [`docs/reports/prewarming-logged-in-caches.md`](reports/prewarming-logged-in-caches.md) |
...
| P125 | open | 2026-09-21 | This deployment's ceiling is between 500 and 1,000 concurrent users, and every wall it has hit so far was a container CPU limit: 175 on 2 app cores, 350 on 4, 500 on a 2-core database, and 1,000 on the same 4 app cores once the database was given 4 of its own | [`docs/PROBLEMS.md`](PROBLEMS.md) |
...
| X26 | holds | 2026-09-21 | Basemap tiles are still ~52% of every request a 500-user hold makes, so that is what a vector basemap removes - of requests, not of cost: X28 re-measured them at 6% of app CPU; the rest of the run's across-the-board improvement is database CPU throttling halving between runs, not a change to any endpoint | [`docs/notes/capacity-after-vector-basemap-measured.md`](notes/capacity-after-vector-basemap-measured.md) |
...
| N28 | current | 2026-09-23 | A read of controllers, services, models, tasks, and the map frontend found game-auth, wiki-notify, geocode, merge, login-oracle, verification-resend, pin-share, undo, profile-preview, trip-list, billing-race, friend-cap, media-buffer, rate-limit, lock, presence, memories-api, achievement-backfill, map-share, billing-sweep, overlay-import, pin-refresh, slide-ready, deletion-mail, decrypt-delete, site-url, broker-fallback, task-time-limit, friend-invite, floorplan-labels, overlay-cap, friend-visibility, link-archive, social-probe, gotify, label-create, trip-location, upload-quota, checksum-race, immich-fetch, gmail-alias, api-key-usage, lost-scan, sso-email, password-reset-lookup, device-scan, lost-device-upload, password-change-tokens, fact-confidence, group-block, trip-list-page, descendant-walk, calendar-fanout, map-center-stale, health-probe, export-memory, session-rows, notification-log, scan-readings, visit-suggestions, public-pin-fanout, backup-dump, group-inbox, invite-mail, notify-mail, channel-buffer, profile-create, map-count, reputation-ledger, custom-field-text, wiki-edit-race, api-page-offset, geolocation-ping, trivia-generation, and saved-filter-scan defects; no fixes proposed | [`docs/notes/codebase-assessment-2026-09-23.md`](notes/codebase-assessment-2026-09-23.md) |
