...
## Pin Restructure
* When viewing a pin details page for a campus that doesn't have any child pins: "Do you want to create child pins for the buildings on this campus?"
...
* Wiki child pins should nest appropriately, without causing "multiple wikis exist for this location". When that situation occurs and one wiki's boundary exists firmly within another wiki's boundary, they should be automatically merged into parent/child wiki pins, without needing user confirmation.
* When multiple official boundaries are available for a location, users should be able to select which boundary is most accurate. For instance: county boundary from REData vs overpass boundary from openstreetmaps. This still prevents users from defining arbitrarily large boundaries that aren't accurate, but allows their input for selecting "correct" data to use for a location. This will be used for the official location boundary for matching pins to wikis. Allow voting, with a weight towards recent votes... so if 0 votes, then REData boundary is used. 1 vote comes in for overpass boundary, then overpass boundary has 1 vote, so it's used. 1 vote comes in for redata boundary, now redata has 1 vote and overpass has 1 vote, the most recent vote is worth slightly more than the older vote, so redata boundary is used. When weighting votes results in an exact tie (like with 0 votes), then default to REData instead of overpass. Use an appropriate algorithm for weighting that factors in "how old". When a user visits a pin detail page or a wiki that has multiple boundary options, and no votes exist, they can be asked via dialog to vote on one. When a consensus exists, stop showing the dialog, but allow users to manually locate a button to cast a vote anyway, if they feel inclined to do so. When they are deciding which boundary is appropriate, they will obviously need a visual display of what that boundary looks like on a map, side-by-side, to make the decision.
