...
class SearchForm(forms.Form):
    ...
    min_priority = forms.IntegerField(required=False, min_value=0, max_value=5)
    max_priority = forms.IntegerField(required=False, min_value=0, max_value=5)
    ...
    min_vulnerability = forms.IntegerField(required=False, min_value=0, max_value=5)
    ...
    def parse_region_geojson(self, key: str) -> MultiPolygon | None:
        ...
        Returns:
            The parsed MultiPolygon, or None when absent or malformed. Never raises - a
            corrupted region payload should drop that one criterion, not fail the whole search.

            "Malformed" now includes "larger than one region may be" (see
            ``geo.MAX_REGION_POLYGONS``), which drops the criterion rather than refusing -
            widening the result instead of narrowing it. Existing contract, not a new one; noted
            here because a filter silently returning *more* than asked for is worth knowing.
        """
        ...
        try:
            ...
            for g in groups:
                ...
                if op in {"and", "or", "not"} and isinstance(ids, list):
                    # str(i).isdigit() returns False for a leading "-", so any negative-number id is
                    # silently dropped. Known, accepted edge case: label PKs are always positive
                    # (Django's default auto-incrementing PK), so this never occurs for a real
                    # label, and this function's contract is to never raise on malformed input.
