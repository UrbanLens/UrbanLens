...
#: Salt for the proxy-URL signature. The photo_name path segment is otherwise fully client-controlled: any
#: logged-in user could replay copied/guessed photo references and consume Places quota through this proxy.
...
#: How long a *confirmed-expired* photo reference is cached as gone, before trying the upstream again - shorter
#: than the success TTL above so a reference that only briefly 404s (rather than being permanently expired -
#: Google's photo references are, in practice, essentially always permanently gone once they 404, but there's no
#: documented guarantee of that) isn't treated as gone forever.
...
class GoogleMapsPhotoProxyView(CredentialOrSessionMediaMixin, View):
    ...
    Accepts either a logged-in session (the browser rendering a pin's Media gallery) or an external API
    credential holding ``media:read`` (the mobile client rendering the same gallery with no session
    cookie at all) - that half is
    :class:`~urbanlens.dashboard.controllers.media_auth.CredentialOrSessionMediaMixin`, shared with the
    authenticated media gate.
    The mixin is called explicitly rather than through ``dispatch()`` because this view's ordering is
    load-bearing - see :meth:`get`.
    """
    ...
    def get(self, request: HttpRequest, photo_name: str) -> HttpResponseBase:
        ...
        - **Signature.** ``photo_name`` is fully client-controlled and this view will fetch it from Google,
          so without the signature the endpoint is an open image-fetc...
        - **Identity.** Only then is the requester resolved, and it is resolved *before* the cache read
          below, which returns real image bytes: gating after it would tu...
        - **The external-lookups opt-out**, which guards only the upstream call - a cache hit costs no
          quota, so an opted-out user still sees imagery the site already ...

        Args:
            request: The current request, carrying either a session or an external API credential.
            photo_name: The Places photo reference from the URL, still percent-encoded as Django's
            ``<path:>`` converter handed it over (untrusted until the...

        Returns:
            The image bytes with the upstream content type; 404 for an unsigned/tampered URL, an expired
            reference, an unresolvable credential, or a...
        """
        # The producer signs the RAW name but reverses the URL with the name percent-encoded (photo names
        # contain slashes), and Django's <path:> converter hands the still-encoded segment through - so accept
        # the signature against either form rather than caring which decoding depth this deployment's URL stack
        # landed on.
        ...
        if not settings.google_unrestricted_api_key and not redata_configured:
            return HttpResponse(status=404)
        # Serving from cache above is free, but an upstream fetch consumes the site's Places quota on this
        # requester's behalf - honor their own external-lookups opt-out for the actual API call.
        ...
        try:
            ...
            if e.response is not None and e.response.status_code == 404:
                # Google Places photo references expire over time (not documented how long they're valid for,
                # but it happens routinely for older cached media) - this is expected, ordinary behavior, not a
