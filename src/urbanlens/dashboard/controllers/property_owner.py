...
def _parse_sale_owner_names(request: HttpRequest) -> tuple[list[str], list[str], str | None]:
    ...
    Returns:
        ``(previous_names, new_names, error)`` - error is None on success, and both lists are empty when
...
class PinOwnerRemoveView(LoginRequiredMixin, View):
    ...
    Unlike the shared/wiki side, there is nowhere else a PinOwner could be "still current" - it belongs
    to exactly this one pin, so removal is a real delete rather than an unlink.
    """
...
def _render_wiki_ownership_panel(request: HttpRequest, location: Location, wiki: Wiki, profile: Profile, error: str | None = None) -> HttpResponse:
    ...
    Officially-sourced owners (looked up from county records via REData) are filtered out for a viewer
    without ``SiteFeature.PROPERTY_OWNERS`` - see ``services.property.owner_access``.
    """
...
class WikiOwnershipPanelView(LoginRequiredMixin, View):
    ...
    def post(self, request: HttpRequest, location_slug: str) -> HttpResponse:
        ...
        # Same case-insensitive dedup the Sale History tab's get_or_create already applies (see
        # WikiPropertySaleTabView.post) - without it this panel and that tab disagree on whether "Alice" and
        # "alice" are the same owner, producing duplicate WikiOwner rows for one real owner.
...
def _render_wiki_sale_tab(request: HttpRequest, location: Location, wiki: Wiki, profile: Profile, error: str | None = None) -> HttpResponse:
    ...
    Party names are filtered the same way the Ownership panel's are: a deed's grantor/grantee written
    from county records is an ``OFFICIAL`` owner, so leaving this tab ungated would hand back exactly
    what that panel withholds.
    """
    from urbanlens.dashboard.services.property.owner_access import sale_rows
    from urbanlens.dashboard.services.wiki.concealment import visible_rows
...
class WikiPropertySaleTabView(LoginRequiredMixin, View):
    ...
    def post(self, request: HttpRequest, location_slug: str) -> HttpResponse:
        ...
        def get_or_create(name: str) -> WikiOwner:
            # visible_rows, not an unfiltered lookup: the sale record this helper feeds is the concealed
            # viewer's own and therefore always rendered back to them - reusing a stranger's invisible WikiOwner
            # would put that stranger's real name/contact directly into the concealed viewer's own,
            # fully-visible Sale History row.
        ...
        # A sale updates who currently owns the property - a seller no longer does (they may still show up
        # elsewhere, e.g. as an owner of a different property, or referenced by this property's own history).
