...
class MediaGateView(CredentialOrSessionMediaMixin, View):
    ...
    An API-shaped request (one carrying an ``Authorization`` header) that fails to authenticate gets 404
    instead, since redirecting an API client to an HTML login form is useless and the redirect itself
    would confirm the path exists.
    Authorization failures raise ``Http404`` rather than 403, deliberately indistinguishable from a file
    that doesn't exist - the same no-oracle policy the wiki access gate follows, so probing media URLs
    can't confirm that a particular file exists but belongs to someone else.
    """
    ...
    def get(self, request: HttpRequest, path: str) -> HttpResponseBase:
        ...
        It is called here rather than applied as a ``xframe_options_sameorigin`` decorator because the two
        origins need different framing rules and a decorator runs after this body - it would overwrite
        whatever the media-origin branch set.

        Args:
            request: The current request, carrying either a session or an external API credential.
            path: The requested path relative to ``MEDIA_ROOT``, straight from the URL (untrusted - may
            attempt traversal).

        Returns:
            An ``X-Accel-Redirect`` response when nginx fronts the app, otherwise a ``FileResponse``
            streaming the file.

        Raises:
            Http404: The path escapes ``MEDIA_ROOT``, the file doesn't exist, or the requester isn't
            authorized to see it.
        """
        # Resolved before the path is touched: _resolve_media_path raises 404 for a nonexistent file, so running
        # it first would let an unauthenticated caller distinguish real paths from invented ones.
        try:
    ...
    def _authorized(self, profile: Profile, rel_path: str) -> bool:
        ...
        Args:
            ...
            rel_path: Normalized path relative to ``MEDIA_ROOT`` (e.g.
            ``"pin_images/a7/Kd3xq.../IMG_4821.jpg"``).
...
class MediaByteSource(ABC):
    ...
    Splitting it this way keeps the path handling (which is security-critical and identical either way)
    in one place, and lets a third store be added by writing one class rather than by adding a branch to
    every caller.
    """
    ...
    def __init__(self, rel_path: str) -> None:
        ...
        Args:
            rel_path: Normalized, traversal-checked path relative to the media root (filesystem) or to the
            bucket (object store).
        """
...
class LocalMediaSource(MediaByteSource):
    ...
    def __init__(self, rel_path: str, full_path: Path) -> None:
        ...
        Args:
            ...
            full_path: The resolved absolute path, already checked to be inside ``MEDIA_ROOT`` and to be an
            existing file.
        """
    ...
    def response(self) -> HttpResponseBase:
        ...
        Returns:
            An ``X-Accel-Redirect`` into ``/_protected_media/`` when nginx fronts the app, otherwise a
            ``FileResponse``.

        Raises:
            Http404: The file was there when the resolver looked and is gone now.
        """
        if getattr(settings, "MEDIA_X_ACCEL", False):
            # Hand the actual byte-serving back to nginx: the internal-only /_protected_media/ location aliases
            # the media volume.
...
class ObjectMediaSource(MediaByteSource):
    ...
    Existence is not checked up front, unlike the filesystem case: on an object store that is a second
    network round trip per media request, and the answer changes nothing.
    A key with no owning row is refused by authorization before this class is reached, and a key that
    vanishes between the two is a 404 raised from :meth:`response` instead of from the resolver.
    """
    ...
    def response(self) -> HttpResponseBase:
        ...
        Returns:
            An ``X-Accel-Redirect`` carrying a URL this process signed - which nginx consumes and strips, so
            it never reaches the client - when...
        ...
        if prefix and callable(signer):
            ...
            # The signed URL rides in a response header nginx removes before it answers the client, so it is a
            # server-to-server credential rather than something the requester ever holds.
...
def _accel_redirect(target: str) -> HttpResponseBase:
    ...
    Args:
        target: The internal location (and, for an object store, the signed URL appended to it) nginx
        should fetch.
...
def resolve_media_path(path: str) -> MediaByteSource:
    ...
    Returns:
        The byte source for the requested file, carrying the normalized POSIX-style path the authorizers
        key off.

    Raises:
        Http404: The path is empty, contains a NUL byte, escapes the media root (traversal attempt), or
        - on the filesystem - isn't an existing file.
    """
...
def serve_media_file(source: MediaByteSource) -> HttpResponseBase:
    ...
    Split out so a surface with its own credential (the safety contact portal authenticates by
    magic-link token, not by login) can reuse the nginx hand-off instead of reimplementing it.

    Args:
        source: The resolved byte source, from :func:`resolve_media_path`.

    Returns:
        Whatever the source's backing store delivers bytes with - an ``X-Accel-Redirect`` response, or a
        ``FileResponse``.
