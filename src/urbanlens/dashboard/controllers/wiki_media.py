...
class WikiMediaVoteView(LoginRequiredMixin, View):
    ...
    An up-vote (``is_relevant: true``) on an externally-sourced item (any ``source`` other than
    ``"photos"``, which already lists real ``Image`` rows attached to this wiki - see
    ``WikiMediaProviderView._photos``) also materializes it and attaches it to this wiki, exactly like the
    pin detail page's "Send to wiki" bulk action - see the module docstring's "An up-vote also submits the
    item to the wiki" bullet for why voting here is treated as that same deliberate sharing action. It
    does *not* cost the voter's storage quota - a cached copy of someone else's photo is exempt, and
    ``materialize_media_item``'s own rolling per-profile ceiling is what bounds it instead. A failed
    download still keeps the vote (the voter's opinion is worth keeping even if today's download attempt
    failed) but is reported back via ``materialize_error`` so the frontend can toast it. A down-vote or a
    cleared vote never materializes anything - only an existing, already-materialized ``image_id`` supplied
    by the client (re-scoped to this wiki's own attached media) gets its REData signal reversed.
    """
