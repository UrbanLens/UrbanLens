...
Comment and trip comment images, label, pin and achievement icons and avatars were always stored as uploaded (P119). They go through the
...
class Command(BaseCommand):
    ...
    help = "Re-encode photos, comment images, icons and avatars already in storage so no metadata stays in the file, keeping photo EXIF and keywords on the row. Run once."
    ...
    def handle(self, *args, **options):
        ...
        if options["dry_run"]:
            self.stdout.write(f"Would re-encode {queryset.count()} stored photo(s), then every published comment image, custom icon and uploaded avatar.")
            return
        ...
        self.stdout.write(f"Comment images, icons and avatars: rewritten or removed {rewritten}, failed {other_failed}.")

    def _reencode_other_images(self) -> tuple[int, int]:
        """Re-encode every published comment image, custom icon and uploaded avatar.
        ...
        shown: list[tuple[str, QuerySet[Any]]] = [
            ("dashboard.Label.custom_icon", Label.objects.exclude(custom_icon="").exclude(custom_icon__isnull=True)),
            ("dashboard.Pin.custom_icon", Pin.objects.exclude(custom_icon="").exclude(custom_icon__isnull=True)),
            ("dashboard.Achievement.custom_icon", Achievement.objects.exclude(custom_icon="").exclude(custom_icon__isnull=True)),
            ("dashboard.Profile.avatar", Profile.objects.exclude(avatar="").exclude(avatar__isnull=True).exclude(avatar__regex=GENERATED_AVATAR_PATTERN)),
        ]
        for key, rows in shown:
            for pk, name in rows.order_by("pk").values_list("pk", key.rsplit(".", 1)[1]).iterator():
                attempt(key, pk, lambda key=key, pk=pk, name=name: reencode_shown(key, pk, name))
