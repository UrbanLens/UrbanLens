...
Every handler resolves its target row and no-ops (logs + returns) if it isn't found yet rather than raising, since Stripe delivers events with no ordering guarantee (e.g."""
...
def sync_from_stripe_subscription(role_subscription: RoleSubscription, stripe_subscription: dict) -> None:
    ...
    Every caller holds that instance across a ``Subscription.retrieve`` round-trip, so a payment landing in that window was being written back out."""
...
def _get_or_create_role_subscription(user: User, role: SubscriptionRole, subscription_id: str, stripe_subscription: dict) -> RoleSubscription:
    ...
    if role.pay_what_you_want:
        # Multiple RoleSubscription rows can exist per (user, role) over time (the uniqueness
        # constraint only applies to non-canceled rows), so a resubscribe after canceling would
        # otherwise start a fresh usage ledger from zero and orphan any banked balance the prior row
        # had earned.
...
def _recover_role_subscription_from_metadata(subscription_id: str) -> RoleSubscription | None:
    ...
    For when a webhook that only ever carries a bare subscription id (never a Checkout Session) is delivered before ``checkout.session.completed`` has created the row - Stripe guarantees neither webhook ordering nor single delivery.
...
def _handle_subscription_updated(stripe_subscription: dict) -> None:
    ...
    # Stripe guarantees neither ordering nor single delivery, and emits `updated` alongside
    # `deleted` at cancellation - so a retried `updated` carrying the pre-cancellation status can
    # arrive after the `deleted` that closed this subscription.
    # Applying it verbatim would hand back whatever access the role grants until the nightly
...
def _handle_invoice_payment_succeeded(invoice: dict) -> None:
    ...
    if role_subscription is None:
        # Stripe guarantees neither webhook ordering nor single delivery, so this can be a genuine
        # race (checkout.session.completed hasn't been processed yet) - not just "unknown".
        # Recovering the row from the Subscription's own metadata is the only chance to bank this
        # payment: nothing ever retries a webhook Stripe has already recorded as delivered, so
        role_subscription = _recover_role_subscription_from_metadata(subscription_id)
        if role_subscription is None:
            return
    else:
        # Refetched rather than read off the invoice: this is the "did this charge clear the bar"
        # moment for dynamic-threshold roles, and the live Subscription is the single source of
        # truth for status/price/period that sync_from_stripe_subscription expects.
        # (The recovery path above already syncs from its own fresh retrieve.)
        stripe_subscription = stripe.Subscription.retrieve(subscription_id).to_dict()
        sync_from_stripe_subscription(role_subscription, stripe_subscription)
...
def _handle_charge_refunded(charge: dict) -> None:
    ...
    if refunds_list.get("has_more"):
        # The embedded page caps at 10 refund objects (Stripe's default list size) - a more-refunded
        # charge needs the rest fetched directly, or every refund past the first page is silently
        # never debited.
        # Since idempotency below is keyed per refund id, a later redelivery of this same
...
def _handle_charge_dispute_closed(dispute: dict) -> None:
    ...
    Only a dispute closing as ``lost`` moves money (Stripe has taken it back); every other closing status leaves the ledger alone."""
