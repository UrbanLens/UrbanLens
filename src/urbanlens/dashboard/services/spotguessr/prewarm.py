...
#: Long enough to cover a player's typical time spent on the preceding round (or, for the solo-start
#: prewarm, browsing the settings dialog before clicking start); short enough that a never-consumed
#: prewarm (an abandoned session, or a player who changes a setting before starting) doesn't sit
#: around indefinitely.
...
def _solo_start_key(profile_id: int, mode: str, config: GameConfig) -> str:
    # Fingerprints the whole config, not just profile+mode - a speculative prewarm computed for one
    # difficulty/geo_bounds must never be served for a different one.
    # A mismatch is simply a cache miss, not a correctness hazard: get_or_create_round falls back to
