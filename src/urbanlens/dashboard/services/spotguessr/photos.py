"""Photo selection for Photos-mode rounds. **Privacy invariant, non-negotiable**: every candidate is gated on ``wiki__isnull=False``, *unless* it's ``solo_profile``'s own pin photo (see ``candidate_image_for_location``'s ``solo_profile`` arg) - the one narrow exception, because the only person who could ever see that round is the pin's own owner."""
...
def locations_with_eligible_photo(location_ids: Iterable[int], *, solo_profile: Profile | None = None) -> list[int]:
    ...
    A bulk pre-filter for ``session.generate_round_content``'s location-retry loop, which otherwise pays one ``candidate_image_for_location`` query per candidate it tries and discards - a profile with plenty of pins but no wiki/own-pin photos anywhere (most commonly a new player) turned that into one query per eligible location, on every Photos-mode round generated (including the homepage's speculative prewarm - see ``tasks.prewarm_spotguessr_solo_start``).
