...
class MapPinPayloadService:
    ...
    def serialize(self, pin: Pin) -> dict[str, Any]:
        ...
        return {
            ...
            "icon": self._effective_icon(pin, badges),
    ...
    @staticmethod
    def _ordered_location_badges(badges: list[Badge]) -> list[Badge]:
        return sorted((b for b in badges if b.kind != "user"), key=lambda b: (-b.order, b.name or ""))

    def _winning_display_badge(self, pin: Pin, badges: list[Badge]) -> Badge | None:
        if pin.custom_icon or pin.icon:
            return None
        for badge in self._ordered_location_badges(badges):
    ...
    def _effective_icon(self, pin: Pin, badges: list[Badge]) -> str | None:
        if pin.custom_icon:
            return pin.custom_icon.url
        if pin.icon:
            return pin.icon
        winning = self._winning_display_badge(pin, badges)
        if not winning:
            return None
    ...
    def _effective_color(self, pin: Pin, badges: list[Badge]) -> str | None:
        if pin.color:
            return pin.color
        if pin.custom_icon or pin.icon:
            return None
