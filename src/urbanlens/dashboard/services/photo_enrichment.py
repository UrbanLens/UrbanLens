...
(``services.enrichment.run_enrichment_cycle``) by ``plugins.builtin.google_places``
...
``LocationCache`` marker row, mirroring ``services.enrichment.AddressEnrichmentSource`` - so a
...
def _save_enriched_image(location: Location, content: bytes, *, source: str, source_url: str = "", caption: str = "", max_dimension: int) -> Image:
    ...
    from urbanlens.dashboard.services.images import compute_checksum, downscale_stored_image
...
class _BackfillMarkerSource(EnrichmentSource):
    ...
    Same contract as ``services.enrichment.AddressEnrichmentSource``: the marker is written
