...
def _suggest_for_pinned_photo(image: Image) -> VisitSuggestion | None:
    ...
    if image.longitude is not None and image.latitude is not None:
        ...
        within_range = Pin.objects.filter(
            pk=pin.pk,
            location__point__dwithin=(photo_point, D(m=PHOTO_VISIT_MATCH_RADIUS_M)),
        ).exists()
        if not within_range:
...
def _suggest_for_unfiled_photo(image: Image) -> VisitSuggestion | None:
    ...
    The suggestion carries ``origin_image`` so accepting it attaches the photo to the resulting visit."""
    from urbanlens.dashboard.models.visit_suggestions.model import VisitSuggestion
