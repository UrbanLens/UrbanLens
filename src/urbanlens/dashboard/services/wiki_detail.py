...
Deliberately shaped like ``services.pin_detail.build_pin_detail`` - same
...
``services.community_counts.wiki_community_summary`` owns the pinned-user count
...
def build_wiki_detail(wiki: Wiki, location: Location, profile: Profile) -> dict[str, Any]:
    ...
    Args:
        wiki: The wiki to serialize. Caller is responsible for the visibility
            ...
            ``services.wiki_access.resolve_visible_wiki``, which every caller
