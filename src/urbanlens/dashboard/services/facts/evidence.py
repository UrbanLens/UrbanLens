...
def record_evidence(
    *,
    ...
    Args:
        ...
        value: The observed value, already shaped for the key's data type (e.g. a ``Point`` for a POINT key).
        ...
        submitter_trust_snapshot: The submitter's trust score at submission time, for sources with a per-submitter trust concept.
        consensus_round: The round this observation came from, if any.
        context: Free provenance metadata.

    Returns:
        The created evidence row, or None if ``key`` isn't registered, or the subject doesn't allow it."""
...
def record_consensus_answer_evidence(round_: ConsensusRound, answer: ConsensusAnswer) -> FactEvidence | None:
    ...
    Returns:
        The created evidence row, or None if this round's field kind isn't Facts-tracked, or the answer has no usable value."""
