...
class UpstreamBreaker(ABC):
    ...
    @abstractmethod
    def scopes(self, url: str) -> tuple[str, ...]:
    ...
    @abstractmethod
    def scope_tripped_by(self, url: str, response: requests.Response) -> str | None:
    ...
    def wait(self, url: str) -> int | None:
        ...
        """
        keys = [self._key(scope) for scope in self.scopes(url)]
        try:
    ...
    def observe(self, url: str, response: requests.Response) -> None:
        ...
        Args:
            ...
            response: The upstream's answer.
        """
        scope = self.scope_tripped_by(url, response)
        if scope is None:
            return
...
class RedataBreaker(UpstreamBreaker):
    ...
    def source(url: str) -> str:
        """The REData endpoint behind ``url``, with its identifiers blanked, which names the providers it draws on.

        Args:
            url: A REData URL.

        Returns:
            The endpoint, such as ``places/search/nearby`` or ``parcels/{id}/demographics``.
        """
    ...
    def scopes(self, url: str) -> tuple[str, ...]:
        ...
        """
        return (*(f"pool:{pool}" for pool in self.pools(url)), f"source:{self.source(url)}")
    ...
    def scope_tripped_by(self, url: str, response: requests.Response) -> str | None:
        ...
        if str(response.headers.get("Retry-After", "")).strip():
            return f"source:{self.source(url)}"
        try:
            body = response.json()
        except ValueError:
            return None
