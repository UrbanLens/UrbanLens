...
def record_tentative_answers(round_: ConsensusRound, distinct_answers: list[ConsensusAnswer]) -> list[ConsensusTentativeAnswer]:
    ...
    Unserialised, both reads miss and both write, which splits one value's support across two rows so it never reaches the promotion threshold, and the row-level ``+=`` loses an increment on top of that."""
...
def _record_coordinate(round_: ConsensusRound, answer: ConsensusAnswer) -> ConsensusTentativeAnswer:
    ...
    # Only ever called for a PHOTO_COORDINATES round's real (non-skip) answers -
    # services.consensus.session guarantees guess_point is set before this runs; the field itself is
    # nullable only because the same column is shared with text-kind answers.
