...
Exposures never reference the recipient's pins, so deleting/re-creating pins cannot clear them, and any future pin within :data:`EXPOSURE_RADIUS_METERS` of an exposed location chains its onward shares back to the originating share."""
...
#: How close (metres) a location must be to an exposed location to count as "the same place" for
#: chain resolution.
#: Matches the map-detection inference radius
#: (services.sharing.map_sharing.INFERRED_SOURCE_SHARE_RADIUS_METERS) but, unlike it, applies with
...
def record_share_exposure(share: PinShare, *, source: ExposureSource = ExposureSource.SHARE_RECEIVED) -> LocationExposure | None:
    ...
    if find_profile_pin_near_location(share.to_profile_id, location) is not None:
        return None
    try:
        # Its own savepoint, scoped to just this write: callers (the share dialog, DM/map detection,
        # trip activities) never wrap their own multi-write sequences in atomic() specifically so
        # that a failure here - swallowed below - can't mark a wider transaction broken and turn a
        # tolerated bookkeeping gap into a hard 500 on the share itself.
...
def profile_is_exposed_to(profile_id: int, location: Location, *, radius_meters: int = EXPOSURE_RADIUS_METERS) -> bool:
    ...
    Used by the detection paths (DM coordinates, trip activities) to avoid piling duplicate detected-share rows onto a place the recipient was already told about - the chain only needs the first one.
...
def resolve_origin_share(profile_id: int, *, pin: Pin | None = None, location: Location | None = None) -> PinShare | None:
    ...
    if pin is not None and pin.location_id is not None:
        from urbanlens.dashboard.services.sharing.map_sharing import infer_source_share_for_pin
