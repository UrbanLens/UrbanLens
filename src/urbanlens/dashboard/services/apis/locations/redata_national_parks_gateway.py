...
This replaces the direct NPS Developer API + ArcGIS boundary-containment lookup this project used before (``services.apis.parks.nps``, now removed): REData has no raw-coordinate containment endpoint of its own (only ``GET /parcels/{uuid}/national-parks/``, keyed by a REData parcel uuid this project doesn't otherwise resolve for most pins), so the panel's semantics shift from "the pin sits inside this park's boundary" to "this is the nearest NPS unit REData's catalog knows about" - see the plugin's own docstring for the tradeoff."""
