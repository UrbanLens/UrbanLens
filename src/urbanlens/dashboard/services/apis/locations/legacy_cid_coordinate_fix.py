...
#: Ceiling on how many of a profile's coordinate-named legacy pins are parsed when looking for the
#: same coordinates written in another format.
#: The verbatim name/alias lookup above it is a plain indexed query and is not capped; this fallback
#: exists only for the rare cross-format case, so it must not turn into an unbounded scan for a user
...
#: "42.1234, -73.5678" or "(42.12 -73.57)".
...
def is_legacy_location(location: Location) -> bool:
    ...
    A Location created before the cutoff may have been created from an S2-decoded guess, so its coordinates can't be trusted to place a pin during a re-import - the CID should be resolved again instead.
...
def repair_legacy_pin_coordinates(
    *,
    ...
    # db_pin_unique_location_per_profile.
    # Leave the legacy pin where it is and suggest merging it into the pin that's already correctly
    # placed instead - merging the two is a user decision, not something an import should do
...
def preview_needs_legacy_repair(profile: Profile, *, cid: int | None, name: str) -> bool:
    ...
    Runs the same matching :func:`repair_legacy_pin_coordinates` uses (CID, then coordinate-shaped name) without needing the corrected coordinates - the import preview only needs to know a legacy pin is sitting there, not yet where it belongs.
...
def _match_by_coordinate_name(profile: Profile, name: str) -> Pin | None:
    ...
    Only applies when the imported name is itself wholly a coordinate - matching ordinary place names by text would be far too loose to move a pin on.
    ...
    # Both the raw name and its sanitized form: the imported name comes straight out of the file,
    # while stored names have been through sanitize_name, so for anything but a plain decimal pair
