...
This gateway backs both the ``yelp`` and ``epa_echo`` plugins - same endpoint, different ``provider=`` value - rather than each plugin getting its own gateway class, mirroring REData's own "one registry, one row per provider" design."""
...
def applicable_provider_tags(latitude: float, longitude: float) -> list[str]:
    ...
    It matters here more than anywhere else in the API: most of these providers are *generated* on REData's side from dataset tables (one per camera register, one per FAA facility group, one per EPA programme), so their tags are not knowable to a client and a hardcoded list would silently stop growing.
