...
#: Plausible calendar-year bound shared by every OHM query in this module - rejects garbage before it
#: reaches Overpass or a caller.
#: Deliberately loose; OHM's own dataset spans a similarly wide range and this is just a plausibility
#: check, not a real historical-coverage bound (that's what
MIN_YEAR = 1000
MAX_YEAR = 2100

#: Matches the leading 1-4 digit year (optionally negative, for BCE dates) at the start of an OHM
#: ``start_date``/``end_date`` value.
#: Real-world values are messy (e.g. an EDTF-flavored "1849~" living in the plain, non-``:edtf`` key)
#: - this only needs the leading year, so it matches and ignores whatever trails it rather than
...
def _extract_year(value: Any) -> int | None:
    ...
    Args:
        value: A ``start_date``/``end_date`` tag value, or anything else (missing tags surface as ``None`` from ``dict.get``).

    Returns:
        The leading year as an int, or None when ``value`` isn't a string or doesn't start with a recognizable year - skipped rather than raised, since OHM's real-world tagging is inconsistent enough that a strict parse would drop otherwise-usable coverage."""
...
def _way_geometry(element: dict[str, Any]) -> dict[str, Any] | None:
    ...
    Returns:
        A GeoJSON geometry dict, or None when ``geometry`` is missing or malformed."""
...
def _element_to_feature(element: dict[str, Any]) -> dict[str, Any] | None:
    ...
    Returns:
        A GeoJSON Feature dict, or None when the element is a relation (multipolygon assembly is out of scope for this pass) or its geometry is missing/malformed - skipped rather than raised, so one bad element doesn't drop the rest of the response."""
...
class OhmCoverage:
    ...
    Attributes:
        available: True when at least one dated (``start_date``/``end_date`` tagged) feature exists within the query radius.
        years: Every distinct year extracted from those features' start and end dates, sorted ascending - a slider tick belongs at both the year something appeared and the year it disappeared."""
