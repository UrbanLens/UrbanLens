...
This gateway is the only thing in UrbanLens that talks to a property-record source: it calls REData's REST API (``GET /api/v1/parcels/lookup/``) and returns the payload shape (REData's own ``PropertyRecord.to_dict()``) that ``plugins.builtin.property_records`` expects."""
...
class RedataGateway(Gateway):
    ...
    def lookup_parcel_uuid(self, latitude: float, longitude: float, *, situs_address: str = "", apn: str = "") -> str | None:
        ...
        Endpoints outside the tiered property-records pipeline itself (e.g. commercial listings) are keyed by REData's own parcel uuid rather than a coordinate - this performs the same lookup as :meth:`lookup_parcel` (REData resolves/caches it identically either way) but returns just the uuid, without assuming anything about ``record_payload``'s shape.
    ...
    def lookup_coverage(self, parcel_uuid: str) -> dict[str, dict[str, Any]]:
        ...
        See REData's ``../REData/docs/api-reference.md``, "GET /parcels/{uuid}/coverage/": every check here is a cheap geographic-bounds or already-known-field test - never a live external fetch - so this is safe to call before deciding whether e.g. :meth:`lookup_assessments` or :meth:`lookup_sale_records` are worth the round trip.
    ...
    def lookup_assessments(self, parcel_uuid: str) -> list[dict[str, Any]]:
        ...
        Rows cover parcels *near* the coordinate, so callers filter by ``parcel_identifier`` for a single parcel's history.
    ...
    def lookup_listings(self, parcel_uuid: str) -> dict[str, Any]:
        ...
        Never fetches from LoopNet inline with the request, even on a cache miss - see the endpoint's own documentation in REData's ``../REData/docs/api-reference.md`` for why (LoopNet's bot-detection, REData's strict outbound budget for it).
    ...
    def lookup_boundaries(self, parcel_uuid: str) -> list[dict[str, Any]]:
        ...
        Deliberately unfiltered: passing any ``?source=`` makes REData skip scoring altogether, leaving ``confidence: 0.0`` and ``is_suggested: false`` on every record - which would discard the only reason to call this rather than picking a polygon ourselves.
