...
The visible tree is built once, here, so both surfaces apply the same three independent gates in the same order:"""
...
class TripReplyData(TypedDict):
    ...
    can_delete: bool
    #: True when this is a reply whose own parent was deleted (UL-219) - such a reply also has
    #: parent=None and so queries identically to a genuine top-level comment; this distinguishes the
    #: two for the template.
