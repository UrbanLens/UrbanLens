...
principle as ``services.pin_creation`` for pin creation.
    ...
    * ``pin_type`` sets ``pin_type_is_user_provided`` to True, so automatic
    ...
    with transaction.atomic():
        ...
        if visited is not None:
            # Imported here rather than at module scope: services.visits imports
            ...
            from urbanlens.dashboard.services.visits import add_visited_status, remove_visited_status
