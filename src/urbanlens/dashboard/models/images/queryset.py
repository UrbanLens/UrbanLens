...
class ImageQuerySet(abstract.FrontendDashboardQuerySet):
    def visible_to(self, viewer_profile: Profile | None) -> Self:
        ...
        # Both gates, and in this order: the photo must have been shared, and the uploader's setting
        # must admit this viewer.
        # Sharing alone is not enough - that is what the setting is for - and a permissive setting
        # alone is not enough either, which is the half that was missing: a photo filed under a pin
    ...
    def _viewer_scoped(profile: Profile, attribute: str, compute: Callable[[], set[int]]) -> set[int]:
        ...
        These three sets describe the *viewer*, not the queryset, so every ``visible_to`` call in one render wants the same answer - and a page can make several.
