...
def award_consensus_points_on_wiki_edit(sender: type[WikiEdit], instance: WikiEdit, created: bool, raw: bool = False, **kwargs) -> None:
    ...
    Fires for every WikiEdit, not just ones created through Consensus, so users get credit for contributions "even if done outside" the game (see the Consensus design).
...
def reconcile_consensus_points_on_revert(sender: type[WikiEdit], instance: WikiEdit, created: bool, raw: bool = False, **kwargs) -> None:
    ...
    Reverting somebody's edit takes back what that edit paid them; reverting the revert puts it back.
    Watching the flag rather than the revert service catches every writer of it, including the Django admin, where ``reverted`` is an editable field on the change form.
...
def record_fact_evidence_on_wiki_edit(sender: type[WikiEdit], instance: WikiEdit, created: bool, raw: bool = False, **kwargs) -> None:
    ...
    Skips Consensus-sourced edits: ``services.consensus.session._finish_round`` already logs evidence for those directly from the submitted answers, before the wiki write even happens - logging here too would double-count the same observation.
