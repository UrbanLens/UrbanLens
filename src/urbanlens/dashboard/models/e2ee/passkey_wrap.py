...
One row per (passkey, bundle): the identity private key wrapped under a secret derived from the WebAuthn ``prf`` extension's output for that credential.
The PRF secret lives in the user's authenticator (or their passkey-sync fabric) - never on this server, never in browser storage - so this is a *convenience* unlock path with the same trust shape as the password wrap: the server stores a blob it cannot open.
"""
...
class E2EEPasskeyWrap(abstract.DashboardModel):
    ...
    ``wrapped_secret`` is a ``crypto_secretbox`` blob (base64: nonce || ciphertext) of the private key under ``HKDF-SHA256(prf_output, info="urbanlens-e2ee-passkey-wrap-v1")``, computed entirely client-side.
    ``bundle_version`` pins the wrap to the keypair it encrypts: ``E2EEResetView`` deletes all wraps inside its atomic swap, and the version stamp is the belt-and-braces backstop should a wrap ever survive one - the keys endpoint serves only wraps matching the bundle's current version.
    """
