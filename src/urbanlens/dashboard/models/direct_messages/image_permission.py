...
class DirectMessageImagePermission(abstract.DashboardModel):
    ...
    Created (as PENDING) the first time a sender attaches an image to a message for a given viewer.
    Until ALLOWED, every image from that sender renders blurred in the viewer's thread - "Allow Once" reveals a single message's images without changing this row (see `DirectMessage.images_revealed`).
    """
