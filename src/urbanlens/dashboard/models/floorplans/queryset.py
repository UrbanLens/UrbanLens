...
class FloorplanQuerySet(FrontendDashboardQuerySet):
    ...
    def at(self, place: Place, on_date: datetime.date | None = None, *, profile=None) -> Floorplan | None:
        ...
        Args:
            ...
            profile: Restrict to this profile's own plans. Callers serving a
                user should always pass one - a plan records door, lock and
                key detail, which is exactly the kind of thing that must not
                leak to everyone who happens to pin the same building.
        ...
        if profile is not None:
            versions = versions.filter(profile=profile)
