...
class PinList(abstract.PublicDashboardModel):
    ...
    def pin_count(self) -> int:
        ...
        ``len(self.items.all())`` rather than ``self.items.count()`` - when a caller has already run ``prefetch_related("items")`` (or ``"items__pin"``) on the queryset this instance came from (see ``PinListsIndexView.get``), ``.all()`` reuses that cached result set instead of issuing a fresh ``COUNT(*)`` query per list on every list-index render.
        """
