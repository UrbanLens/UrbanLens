...
class Comment(LocationMentioningModel, abstract.FrontendDashboardModel):
    ...
    # True from creation until the async malware scan (tasks.scan_comment_image) clears a
    # newly-uploaded image - never set for a comment with no image, or one attached via "Choose
    # Existing" (already scanned on its original upload).
    # While True, the comment is visible only to its own author (see
