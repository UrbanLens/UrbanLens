...
def flag_replies_on_parent_delete(sender: type[Comment], instance: Comment, **kwargs) -> None:
    ...
    Runs pre-delete (rather than post-delete) so the affected replies are flagged before Django's collector nulls out their ``parent`` FK via ``on_delete=SET_NULL`` - by post-delete time there is no reliable way to find them again (mirrors ``markup.signals.flag_map_removed_on_map_delete``).
    """
