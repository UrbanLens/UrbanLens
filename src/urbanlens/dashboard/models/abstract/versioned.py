...
One row per ``(target, field, write)``.
That granularity is what lets a viewer be shown a *subset* of the history - automatic writes, plus their own, plus their friends' - as a single indexed query rather than a replay:
"""
...
class AbstractFieldRevision(Model):
    ...
    Concrete subclasses add ``target`` as a real ForeignKey to their own model - per-model tables rather than one contenttypes table, so each gets a typed FK, its own indexes and its own retention policy.
    ...
    #: Django gives concrete subclasses a default manager either way; naming it here is what lets a
    #: helper return `type[AbstractFieldRevision]` and use it, instead of returning `type[Model]`
    #: and being unable to query.
    objects = Manager()
...
class VersionedQuerySet(QuerySet):
    ...
    ``update()`` is the reason this class exists.
    It bypasses ``save()`` *and* every signal, so a model-level hook cannot see it - which is exactly how the three existing bypasses became invisible.
    """
    ...
    def update(self, **kwargs: Any) -> int:
        ...
        # Snapshot the pks before the write - the update may change the very fields this queryset
        # filters on, so re-evaluating afterwards can match a different set of rows - but do it
        # under a lock, and inside the same transaction as the write.
        # Without that, a compare-and-set loses silently in the worst possible direction.
    ...
    def bulk_create(self, objs: Sequence[Any], *args: Any, **kwargs: Any) -> list[Any]:
        ...
        A create is revision 1: the whole versioned field set is the state the row started in, with whatever source the context says made it.
        Without this, a bulk-created row has no provenance at all, and every one of its fields resolves to a default for a concealed viewer - which reads as an empty wiki rather than the one that was actually created.
        """
    ...
    def bulk_update(self, objs: Sequence[Any], fields: Sequence[str], batch_size: int | None = None) -> int:
        ...
        # `queryset.filter(pk__in=...).update(**case_expressions)` on a clone of this very class, so
        # `update()` above re-enters and would record a `Case` expression's repr as the value.
        # Suppress that inner pass and record the real values here.
...
class VersionedModel(Model):
    ...
    Every write to a listed field records who made it, so that a filtered view of the field set can be assembled later for a viewer entitled to only part of it.
    """
    ...
    def save(self, *args: Any, **kwargs: Any) -> None:
        ...
        Recording *every* versioned field on an untargeted save would be worse than noisy - it would be wrong.
        """
...
def _record_fields(model: type[Model], pk: Any, values: dict[str, Any]) -> None:
    ...
    try:
        ...
        # Its own savepoint.
        # On PostgreSQL a failed INSERT aborts the whole transaction at the server, so catching the
        # Python exception is not enough - the caller's next statement would fail with "current
        # transaction is aborted", and several callers (wiki_creation, pin_wiki_sync, the external
...
def concrete_field(model: type[Model], field_name: str) -> Field | None:
    ...
    try:
        ...
        # `field_name` is historical text in an append-only table, so a field that is later renamed
        # or dropped leaves rows naming a column that no longer exists.
        # Without this, the first such rename turns every resolve_fields call on an affected row
        # into a 500 on the concealed render path.
        return None
    return field if isinstance(field, Field) else None


def _serialise(model: type[Model], field_name: str, value: Any) -> str:
    """Render a field value as text for storage. Foreign keys store their pk."""
...
def resolve_fields(
    ...
    *,
    ...
    for row in rows:
        ...
        if field is None:
            continue
        # A relation stores its pk; everything else goes back through the field's own to_python,
        # which is what stops this inheriting the existing revert path's bug of assigning a
        # stringified value onto a typed field.
...
def purge_recorded_value(target: Model, field_name: str, value: Any) -> int:
    ...
    The revision log is append-only by design, and that is at odds with one existing user-facing promise: permanently deleting your own wiki edit ("intended for cases like accidentally pasting private information into a public wiki field") is supposed to leave *no copy anywhere*.
