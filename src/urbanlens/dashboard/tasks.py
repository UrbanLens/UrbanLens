...
def _process_photo_upload(image: Image, image_id: int, strip_location: bool, max_dimension_override: int | None = None) -> _UploadProcessResult | None:
    ...
    if image.profile is not None:
        ...
        # A profile-less row (location enrichment) has no plan to read a policy from; its caller passes the cap
        # instead.
    ...
    if downscale_policy is not None:
        ...
        try:
            ...
            # DecompressionBombError inherits straight from Exception, not from OSError/ValueError like the rest
            # of Pillow's failures (Unidentified- ImageError does), so it escaped this handler and took the
            # whole photo-processing task down with it.
...
def sweep_immich_library_locations(self, profile_id: int) -> dict[str, int]:
    ...
    try:
        for page, _page_total in gateway.iter_library_assets():
            ...
            # library_total is the true library-wide count (see library_asset_count) - unlike the deprecated
            # per-page "total" iter_library_assets also yields, which mirrors the current page size and would
            # make this message read "Scanned 194000 of 1000" once scanned outgrows a single page.
            if library_total:
...
def resolve_deferred_pin_locations(
    ...
    Queued by ``GoogleMapsGateway.import_preview_streaming`` for any confirmed pin whose cid had neither
    an existing Location nor a cached Places lookup - see that method's docstring for why the preview's
    own lat/lng can't be trusted for these.

    Args:
        profile_id: PK of the importing profile.
        deferred_lists: Same shape as import_preview_streaming's confirmed_lists, restricted to pins
        needing a live cid lookup - shrinks on each retry to just...
        auto_tag: Whether to enqueue AI category suggestion for newly-created pins.
...
# No autoretry, deliberately: the beat scheduler re-fires this every hour anyway, and a retry racing the next
# scheduled run would double-spend the API budget the cycle just computed.
...
# No autoretry here, deliberately: run_panel_fetch owns the failure policy (suppression markers with their own
# TTLs), and Celery-level retries would race the poll-driven re-scheduling in schedule_panel_fetch.
...
def process_device_scan_upload(self, upload_id: int) -> bool:
    ...
    Always marks the upload PROCESSED or FAILED by the time this returns, even on an unexpected error,
    so a stuck PENDING row always means the task never ran at all rather than having failed silently
    mid-way.
