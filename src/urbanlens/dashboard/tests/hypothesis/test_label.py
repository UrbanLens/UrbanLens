...
class LabelDescendantSeedTests(TestCase):
    """get_label_and_descendants always includes the seed."""

    def test_seed_id_always_in_result(self) -> None:
        tag = _make_tag()
        result = Label.get_label_and_descendants(tag.pk)
        self.assertIn(tag.pk, result)

    def test_leaf_label_returns_singleton(self) -> None:
        """A label with no children should return exactly {label_id}."""
        tag = _make_tag()
        result = Label.get_label_and_descendants(tag.pk)
    ...
    def test_multi_level_chain_all_included(self, depth: int) -> None:
        ...
        result = Label.get_label_and_descendants(nodes[0].pk)
        self.assertEqual(result, {n.pk for n in nodes})


class LabelCycleTests(TestCase):
    ...
    def test_two_node_cycle_terminates(self) -> None:
        ...
        result_a = Label.get_label_and_descendants(tag_a.pk)
        result_b = Label.get_label_and_descendants(tag_b.pk)
    ...
    def test_three_node_cycle_terminates(self) -> None:
        ...
        for node in nodes:
            result = Label.get_label_and_descendants(node.pk)
            self.assertEqual(result, {n.pk for n in nodes})


class LabelDescendantMonotonicityTests(TestCase):
    ...
    def test_adding_child_never_reduces_result_size(self, n_extra: int) -> None:
        parent = _make_tag(name="parent")
        initial = Label.get_label_and_descendants(parent.pk)
        for i in range(n_extra):
            child = _make_tag(name=f"added_{i}")
            child.parents.add(parent)
        after = Label.get_label_and_descendants(parent.pk)
...
class LabelResultTypeTests(TestCase):
    """get_label_and_descendants must always return a set of ints."""
