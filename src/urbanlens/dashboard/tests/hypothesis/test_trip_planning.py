...
class ComputeLegsTests(TestCase):
    ...
    def test_live_calls_respect_the_budget_and_fill_the_cache(self) -> None:
        with patch("urbanlens.dashboard.services.trips.trip_legs.get_route_between") as get_route_between:
            get_route_between.return_value = {"distance_meters": 5000.0, "duration_seconds": 300.0}
            legs = compute_legs(self.stops, max_live_calls=1)
        self.assertEqual(get_route_between.call_count, 1)
        self.assertEqual(set(legs), {2})

        # Second render: leg 1→2 is cached now, so the budget covers 2→3.
        with patch("urbanlens.dashboard.services.trips.trip_legs.get_route_between") as get_route_between:
            get_route_between.return_value = {"distance_meters": 7000.0, "duration_seconds": 420.0}
            legs = compute_legs(self.stops, max_live_calls=1)
        self.assertEqual(get_route_between.call_count, 1)
        self.assertEqual(set(legs), {2, 3})
    ...
    def test_unroutable_pairs_are_negative_cached(self) -> None:
        with patch("urbanlens.dashboard.services.trips.trip_legs.get_route_between") as get_route_between:
            get_route_between.return_value = None
