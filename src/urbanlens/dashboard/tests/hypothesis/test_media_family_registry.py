...
_FILENAMES = st.text(
    alphabet=st.characters(blacklist_characters="/\\\x00", blacklist_categories=("Cs",)), min_size=1, max_size=200
)
...
class MediaFamilyCheckTests(SimpleTestCase):
    ...
    def test_every_media_field_in_the_project_is_covered(self):
        ...
        self.assertEqual(
            check_media_authorizers(), [], "a model stores files under a directory the media gate would refuse"
        )
    ...
    def test_an_unregistered_directory_is_reported(self):
        ...
        self.assertFalse(
            authorize_media(_AnyProfile(), "brand_new_family/leaked.png"),
            "an unregistered family must be refused, not served",
        )
...
class UnguessableUploadPathTests(SimpleTestCase):
    ...
    @given(
        filename=st.sampled_from(
            [
                "PXL_20260709_123456.jpg",
                "IMG_4821.JPG",
                "MVIMG_20260709_1.jpg",
                "DSCN0042.jpeg",
                "DSC00042.jpeg",
                "DCIM_1234.jpg",
                "abandoned-hospital-stairwell.jpg",
            ]
        )
    )
