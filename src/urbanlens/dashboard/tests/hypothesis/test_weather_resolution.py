...
class GetForecastSlotsTests(SimpleTestCase):
    def test_redata_configured_prefers_openweathermap_entry(self) -> None:
        results = [
            {"provider": "open_meteo", "forecast": [{"granularity": "hourly", "starts_at": "2026-06-15T09:00:00", "temperature_c": 10.0, "condition": "clear"}]},
            {"provider": "openweathermap", "forecast": [{"granularity": "hourly", "starts_at": "2026-06-15T09:00:00", "temperature_c": 20.0, "condition": "rain"}]},
        ]
        with (
    ...
    def test_redata_not_configured_uses_direct_chain(self) -> None:
        with (
            ...
            mock.patch("urbanlens.dashboard.services.apis.weather.open_meteo.OpenMeteoGateway.get_weather_forecast", return_value=[]) as open_meteo,
        ):
    ...
    def test_openweathermap_configured_takes_priority_over_open_meteo_on_direct_path(self) -> None:
        ...
        with (
            ...
            mock.patch("urbanlens.dashboard.services.apis.weather.gateway.OpenWeatherMapGateway.get_weather_forecast", return_value=[raw_item]),
            mock.patch("urbanlens.dashboard.services.apis.weather.open_meteo.OpenMeteoGateway.get_weather_forecast") as open_meteo,
        ):
            slots = weather_resolution.get_forecast_slots(1.0, 2.0)
...
class GetSunTimesTests(SimpleTestCase):
    def test_redata_configured_prefers_open_meteo_entry(self) -> None:
        results = [
            ...
            {"provider": "open_meteo", "sun": {"sunrise": "2026-06-15T05:32:00", "sunset": "2026-06-15T20:47:00", "golden_hour_morning_end": "2026-06-15T06:32:00", "golden_hour_evening_start": "2026-06-15T19:47:00"}},
        ]
        with (
            mock.patch(f"{_MODULE}.redata_configured", return_value=True),
            mock.patch(f"{_MODULE}.RedataWeatherGateway") as gateway_cls,
    ...
    def test_redata_not_configured_uses_open_meteo_directly(self) -> None:
        with (
            mock.patch(f"{_MODULE}.redata_configured", return_value=False),
            mock.patch("urbanlens.dashboard.services.apis.weather.open_meteo.OpenMeteoGateway.get_sun_times", return_value=None) as open_meteo,
        ):
