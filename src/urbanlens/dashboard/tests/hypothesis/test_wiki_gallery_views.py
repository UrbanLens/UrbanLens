...
class WikiGalleryNoWikiTests(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.unrelated_image = baker.make(Image, pin=other_pin, wiki=None, profile=self.profile)
...
class WikiGalleryScopingTests(TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.own_image = baker.make(Image, wiki=self.wiki, location=self.location, profile=self.profile, latitude="1.0", longitude="2.0")

        other_pin = baker.make(Pin, profile=self.profile)
        self.unrelated_image = baker.make(Image, pin=other_pin, wiki=None, profile=self.profile, latitude="3.0", longitude="4.0")
