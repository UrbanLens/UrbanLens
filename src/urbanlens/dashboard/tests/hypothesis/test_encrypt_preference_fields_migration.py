...
class EncryptPreferenceFieldsMigrationTests(TestCase):
    ...
    def test_contact_label_survives_the_retrofit(self) -> None:
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                "UPDATE dashboard_safety_contact_defaults SET label = %s WHERE id = %s", ["Dana (sister)", contact.pk]
            )
    ...
    def test_invitation_note_survives_the_retrofit(self) -> None:
        invitation: FriendInvitation = baker.make(
            FriendInvitation, inviter=baker.make(User).profile, email="newcomer@example.com", message=None
        )
        with connection.cursor() as cursor:
            cursor.execute(
                "UPDATE dashboard_friendinvitation SET message = %s WHERE id = %s",
                ["come explore with us", invitation.pk],
            )
    ...
    def test_the_column_really_holds_ciphertext_afterwards(self) -> None:
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                "UPDATE dashboard_profiles SET additional_preferences = %s WHERE id = %s", ["ask first", profile.pk]
            )
