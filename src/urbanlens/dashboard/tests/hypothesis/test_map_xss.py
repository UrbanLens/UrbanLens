...
Invariants verified:
  - A Badge (tag/category) name is never embedded raw into the `<script>` blocks
    that build `filter_badges_json` (map/index.html, view_map) or `tags_data_json`
    ...
    in a badge name would let stored badge data break out of the script and
...
class FilterBadgesJsonXssTests(_MapXssTestCase):
    """view_map ("/dashboard/map/") embeds `filter_badges_json` inline via `|safe`."""

    def test_malicious_badge_name_is_not_embedded_raw(self) -> None:
        baker.make(Badge, name=_SCRIPT_BREAKOUT_PAYLOAD, kind=KIND_TAG, profile=None)

        resp = self.client.get(reverse("map.view"))

        body = resp.content.decode()
        self.assertNotIn(_SCRIPT_BREAKOUT_PAYLOAD, body)
        # The badge name must still be present, just safely encoded for a JS/JSON context.
...
class TagsDataJsonXssTests(_MapXssTestCase):
    ...
    def test_malicious_tag_name_is_not_embedded_raw(self) -> None:
        ...
        badge = baker.make(Badge, name=_SCRIPT_BREAKOUT_PAYLOAD, kind=KIND_TAG, profile=None)
