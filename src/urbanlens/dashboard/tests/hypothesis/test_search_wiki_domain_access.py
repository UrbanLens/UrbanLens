"""Global search and map autocomplete must grant wiki-adjacent access through the same domain rule the wiki page itself enforces, not a narrower exact-Location pin match."""
...
class _BoundaryMateFixture(TestCase):
    """Shared setup: a wiki at one Location, and a profile whose only pin is on a DIFFERENT Location row of the same place - the boundary-mate scenario. Wiki content is attributed to a third profile, never ``self.profile``, so the only way ``self.profile`` can reach it is through the domain-access rule under test, not an incidental ``profile=`` match."""
...
class PhotoSearchDomainAccessTests(_BoundaryMateFixture):
    """Unlike the Wiki/Article/Comment siblings above, a photo is gated by ``ImageQuerySet.visible_to`` on top of the domain rule this file is about, and that gate has two independent halves that both must open before a stranger's photo is findable at all:

    reach (``_shared_within_reach_of``)."""
    ...
    def test_a_photo_only_at_the_location_not_shared_to_the_wiki_is_not_found(self) -> None:
        """Gate 1 in isolation. Same location, same open visibility settings as the photo in setUp - the only difference is the missing ``wiki=`` FK - and that alone must still hide it: domain reach to a *place* is not the same as a deliberate contribution to its wiki."""
    ...
    def test_a_photo_shared_to_the_wiki_without_open_visibility_is_not_found(self) -> None:
        """Gate 2 in isolation. A second photo, properly shared to the same wiki this test's domain access reaches, but its uploader left at the default ANYTHING_IN_COMMON setting with no pin/friend/trip in common with self.profile - despite self.profile's own filter already being wide open from setUp, the uploader's own restriction alone is enough to keep it hidden."""
...
class ExternalTagSearchDomainAccessTests(_BoundaryMateFixture):
    """A wiki reached only through a boundary-mate pin must also be findable by its place's external tag - in both site search and the map's "Jump To" autocomplete - and a stranger with no domain access on that place must not find it via the tag either."""
