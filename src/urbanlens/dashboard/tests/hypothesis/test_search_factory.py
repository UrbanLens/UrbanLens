"""Tests for the search provider factory and _build_pin_search_query.
...
class BuildPinSearchQueryNameTests(TestCase):
    ...
    def test_name_only(self):
        result = _build_pin_search_query(_pin(effective_name="Riverside Mill"))
        self.assertEqual(result, "Riverside Mill")

    def test_name_and_state(self):
        result = _build_pin_search_query(_pin(effective_name="Old Hospital", state="OH"))
    ...
    def test_name_and_city_and_state(self):
        result = _build_pin_search_query(_pin(effective_name="Steel Works", city="Pittsburgh", state="PA"))
    ...
    def test_address_added_when_different_from_name(self):
        result = _build_pin_search_query(_pin(effective_name="The Mill", address_basic="123 Mill Rd"))
        self.assertIn("123 Mill Rd", result)

    def test_address_not_repeated_when_same_as_name(self):
        result = _build_pin_search_query(_pin(effective_name="123 Mill Rd", address_basic="123 Mill Rd"))
        self.assertEqual(result.count("123 Mill Rd"), 1)

    def test_place_name_not_repeated_when_same_as_name(self):
        result = _build_pin_search_query(_pin(effective_name="River Mill", place_name="River Mill", state="OH"))
    ...
    def test_county_used_when_city_absent(self):
        result = _build_pin_search_query(_pin(effective_name="Farm", county="Erie County"))
        self.assertIn("Erie County", result)

    def test_city_preferred_over_county(self):
        result = _build_pin_search_query(_pin(effective_name="Farm", city="Cleveland", county="Cuyahoga County"))
    ...
    def test_no_data_with_coords_returns_lat_lng(self):
        result = _build_pin_search_query(_pin(latitude=40.7, longitude=-74.0))
        self.assertEqual(result, "40.7, -74.0")

    def test_coords_not_used_when_other_data_present(self):
        result = _build_pin_search_query(_pin(effective_name="Mill", latitude=40.7, longitude=-74.0))
        self.assertNotIn("40.7", result)

    def test_output_is_comma_separated(self):
        result = _build_pin_search_query(_pin(effective_name="Mill", city="Akron", state="OH"))
    ...
    def test_no_leading_or_trailing_commas(self):
        result = _build_pin_search_query(_pin(effective_name="Mill", state="OH"))
    ...
    def test_result_contains_name_and_state(self, name: str, state: str):
        result = _build_pin_search_query(_pin(effective_name=name, state=state))
    ...
    def test_coords_fallback_contains_both_values(self, lat: float, lng: float):
        result = _build_pin_search_query(_pin(latitude=lat, longitude=lng))
