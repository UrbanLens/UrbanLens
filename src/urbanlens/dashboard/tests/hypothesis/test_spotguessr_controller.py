...
class SpotGuessrStartViewTests(TestCase):
    ...
    def test_requires_login(self) -> None:
        self.client.logout()
        response = self.client.post("/spotguessr/start/", {})
        self.assertEqual(response.status_code, 302)

    def test_starting_a_session_returns_a_round_without_leaking_the_answer(self) -> None:
        response = self.client.post("/spotguessr/start/", {"total_rounds": "3"})
    ...
    def test_invalid_difficulty_is_rejected(self) -> None:
        response = self.client.post("/spotguessr/start/", {"difficulty": "not-a-number"})
        self.assertEqual(response.status_code, 400)

    def test_invalid_geo_bounds_json_is_rejected(self) -> None:
        response = self.client.post("/spotguessr/start/", {"geo_bounds": "{not valid json"})
        self.assertEqual(response.status_code, 400)

    def test_geo_bounds_that_isnt_a_geometry_is_rejected(self) -> None:
        response = self.client.post("/spotguessr/start/", {"geo_bounds": json.dumps({"foo": "bar"})})
        self.assertEqual(response.status_code, 400)
...
class SpotGuessrGuessFlowTests(TestCase):
    def setUp(self) -> None:
        ...
        baker.make(Image, location=self.location, media_type=MediaKind.PHOTO, latitude=None, longitude=None)
        self.client.force_login(self.profile.user)
        start = self.client.post("/spotguessr/start/", {"total_rounds": "1"}).json()
    ...
    def test_guessing_reveals_the_location_and_score(self) -> None:
        response = self.client.post(
            f"/spotguessr/session/{self.session_id}/round/{self.round_id}/guess/",
    ...
    def test_a_second_guess_on_the_same_round_is_rejected(self) -> None:
        ...
        self.client.post(f"/spotguessr/session/{self.session_id}/round/{self.round_id}/guess/", payload)
        response = self.client.post(f"/spotguessr/session/{self.session_id}/round/{self.round_id}/guess/", payload)
        self.assertEqual(response.status_code, 400)

    def test_missing_coordinates_are_rejected(self) -> None:
        response = self.client.post(f"/spotguessr/session/{self.session_id}/round/{self.round_id}/guess/", {})
        self.assertEqual(response.status_code, 400)
    ...
    def test_summary_reports_the_final_score(self) -> None:
        ...
        response = self.client.get(f"/spotguessr/session/{self.session_id}/summary/")
        self.assertEqual(response.status_code, 200)
...
class SpotGuessrPinsViewTests(TestCase):
    def test_only_returns_the_requesting_profiles_own_pins(self) -> None:
        ...
        response = self.client.get("/spotguessr/pins/")
        self.assertEqual(response.status_code, 200)
...
class SpotGuessrSettingsViewTests(TestCase):
    def test_toggling_show_ratings_to_friends(self) -> None:
        ...
        response = self.client.post("/spotguessr/settings/", {"show_ratings_to_friends": "off"})
        self.assertEqual(response.status_code, 200)
