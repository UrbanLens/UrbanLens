...
class LedgerStaleSnapshotTests(TestCase):
    ...
    def test_a_payment_is_not_rolled_back_by_the_daily_sweep(self) -> None:
        ...
        A payment landing inside that window must survive."""
    ...
    def test_the_caller_s_instance_reflects_what_was_stored(self) -> None:
        ...
        ``_handle_invoice_payment_succeeded`` reads nothing after ``apply_payment`` today, but an instance that
        silently disagrees with the row it came from is a trap for the next caller."""
    ...
    def test_advance_usage_ledger_syncs_the_caller_s_instance(self) -> None:
        """The same trap as above, for the two fields ``advance_usage_ledger`` itself writes - the prior test only ever checks ``total_paid_cents`` on the caller's object, which ``apply_payment`` writes directly. ``amount_used_cents`` and ``usage_covered_until`` are only ever set on ``locked``, so a sync-back that silently dropped them from the post-lock copy loop would pass every other test in this file while leaving direct callers holding stale coverage."""
        subscription = self._snapshot()
