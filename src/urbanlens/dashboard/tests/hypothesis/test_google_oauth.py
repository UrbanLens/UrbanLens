...
class TokenExchangeTests(SimpleTestCase):
    def test_successful_exchange_returns_the_payload(self) -> None:
        ...
        with patch("urbanlens.dashboard.services.auth.google_oauth.requests.post", return_value=response) as mock_post:
    ...
    def test_revoke_reports_google_confirmation(self) -> None:
        with patch("urbanlens.dashboard.services.auth.google_oauth.requests.post", return_value=Mock(status_code=200)):
            self.assertTrue(revoke_token("token"))
        with patch("urbanlens.dashboard.services.auth.google_oauth.requests.post", return_value=Mock(status_code=400)):
            self.assertFalse(revoke_token("token"))
