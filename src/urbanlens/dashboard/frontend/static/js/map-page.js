...
const MAP_CFG = JSON.parse(document.getElementById('map-page-config').textContent);
    ...
    function _ulEscText(value) {
        return String(value == null ? '' : value).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
    }
    function _ulEscAttr(value) {
    ...
    const _changedLabelIds = new Set();
    ...
    let   _lastClickedPinUuid = null;

    function _isAdditiveClick(e) {
        const orig = e && e.originalEvent;
        return !!(orig && (orig.ctrlKey || orig.metaKey));
    }

    function _handleSelectablePinClick(e, uuid, marker) {
    ...
    if (_MAP_CENTER_MODE !== 'gps' && navigator.geolocation && navigator.permissions?.query) {
        navigator.permissions.query({ name: 'geolocation' }).then(function (status) {
            ...
            navigator.geolocation.getCurrentPosition(
                ...
                function () { /* a refused or failed passive read is not worth surfacing */ },
                { maximumAge: 300000, timeout: 8000 },
            );
        }).catch(function () { /* Permissions API unavailable - stay with the cached dot */ });
    }
    ...
    const _CACHE_FIELDS = new Set([
        ...
        'latitude', 'longitude',
        'description', 'last_visited', 'rating',
        'label_ids', 'viewLocationUrl', 'address', 'cover_photo_url',
    ]);
    ...
    function _readCache() {
        ...
        try {
            ...
            if (c.v !== 11 || c.profileUuid !== _PROFILE_UUID) { localStorage.removeItem(_CACHE_KEY); return null; }
    ...
    // Delegates to shared/pin-cache.ts (installed on window by core.js, which
    // base.html loads synchronously in <head>). Falls back to a no-op rather than
    // throwing if that bundle ever fails to load - reclaiming space is an
    // optimization, never a correctness requirement.
    function _purgeOrphanCaches() {
        try {
            const removed = window.ulPurgeForeignPinCaches ? window.ulPurgeForeignPinCaches(_CACHE_KEY) : 0;
    ...
    function _writeCache() {
        ...
        const payload = JSON.stringify({
            v: 11, ts, profileUuid: _PROFILE_UUID, appUuid: _APP_UUID,
        ...
        try {
            ...
            try {
                ...
                console.warn('[UL] Cache too large for localStorage (%s) - pins will reload next visit', retryErr && retryErr.name || retryErr);
                try { localStorage.removeItem(_CACHE_KEY); } catch {}
                ...
                if (!_filterMode) {
                    if (!existing) {
                        ...
                        const changed = existing.icon !== pin.icon || existing.color !== pin.color
                            || _labelsDiffer(existing, pin) || existing.rating !== pin.rating
    ...
    function _placeFloatingMenu(menu, clientX, clientY) {
        window.MapContextMenu.place(menu, clientX, clientY);
    }
    ...
    function _markerMenuAnchor(e, lat, lng) {
    ...
    function _showSearchMarker(lat, lng, title) {
        ...
        _searchMarker.on('click', function(e) {
            ...
            window.MapContextMenu.show({
                ...
                clientY: anchor.y,
                extraItems: [
                    { icon: 'add_location', label: 'Create pin', onClick: function () { _clearSearchMarker(); openAddPinDialog(lat, lng, { defaultName: title || '' }); } },
                    { icon: 'close', label: 'Dismiss marker', onClick: function () { _clearSearchMarker(); } },
                ],
            });
        });
    ...
    function _mergeLabels(labels) {
        if (!labels || typeof labels !== 'object') return;
        const merged = Object.assign({}, _labelDict);
    ...
    function _labelsDiffer(before, after) {
    ...
    function _pinTagObjects(pin) {
        if (Array.isArray(pin.label_ids)) {
            return pin.label_ids.map(id => _labelDict[String(id)]).filter(Boolean);
        }
        ...
        return String(pin.tags || '').split(',').map(s => s.trim()).filter(Boolean).map(n => ({ name: n }));
    }
    ...
    function _buildMarker(pin) {
        ...
        let tagChipsData = _pinTagObjects(pin);
                ...
                <div class="popup-actions">
                    <div class="popup-menu">
                        ...
                        <div class="popup-menu-items" role="menu" hidden>
                            ...
                            ${pin.child_count > 0 ? `<button type="button" role="menuitem" class="promote-children-button" onclick="event.stopPropagation(); closePinPopupMenus(); promoteChildPins('${pin.slug || pin.uuid}', ${pin.child_count})"><i class="material-symbols-outlined">move_up</i> Promote child pins</button>` : ''}
    ...
    // Thin shim over the shared helper (frontend/ts/shared/fetch-json.ts), which
    // this page's own version was promoted into. Kept so the timeout stays a
    // positional argument at the call sites below.
    function _fetchJson(url, options, timeoutMs) {
        return window.ulFetchJson(url, Object.assign({}, options || {}, { timeoutMs: timeoutMs || 120000 }));
    }

    function _setLoadProgress(text) {
    ...
    async function _fetchMapDocument(message) {
        ...
        try {
            ...
            const pins = [];
            let buffer = '', head = null, ended = false;
            for (;;) {
            ...
            return { pins, next_cursor: null, total: head.total };
        } finally {
            clearTimeout(timer);
        }
    }

    async function _fetchEveryPin(message) {
        try {
    ...
    if (_MAP_CENTER_MODE === 'remember') {
        ...
        let _pendingPositionBody = null;
        function _positionBody() {
            const c = map.getCenter();
            const z = map.getZoom();
    ...
    function _syncFilterStoreClaim() {
        const field = document.getElementById('fp-store-fingerprint');
        if (!field) return;
        const usable = _filterStoreTrusted && _fetchedTiles.has('*') && _lastKnownUpdated;
        field.value = usable ? _lastKnownUpdated : '';
    }

    function _pushFilterStateToUrl() {
        const form = document.getElementById('filter-form');
        if (!form) return;
        const params = new URLSearchParams(new FormData(form));
        const clean = new URLSearchParams();
        for (const [k, v] of params.entries()) { if (v !== '' && k !== 'store_fingerprint') clean.append(k, v); }
        history.replaceState({ filter: clean.toString() }, '', _urlWithMapView(clean));
    }

    function _applyFilterMeta(meta) {
    ...
    function _refilterWithPayloads(missing) {
        ...
        if (_filterStoreMisses === 1) {
            _refreshAllPins().then(function () { _filterStoreTrusted = true; _syncFilterStoreClaim(); }).catch(function () {});
        }
    }
        ...
        marker.bindPopup(`
            <div class="pin-popup child-pin-popup" data-uuid="${pin.uuid}">
                ...
                <div class="popup-actions">
                    ...
                    ${pin.child_count > 0 ? `<button class="btn btn--icon promote-children-button" onclick="event.stopPropagation(); promoteChildPins('${pin.slug || pin.uuid}', ${pin.child_count})" title="Promote all child pins up one level"><i class="material-symbols-outlined">move_up</i></button>` : ''}
                </div>
            </div>`);
    ...
    function _loadChildPins() {
        ...
        _childPinsFetchPromise = _fetchJson(url, { headers: { 'X-Requested-With': 'XMLHttpRequest' } }, 30000)
            ...
            .catch(function (err) { console.warn('[UL] Could not load child pins:', err); })
    ...
    document.getElementById('filter-form').addEventListener('htmx:configRequest', function (e) {
        _syncFilterStoreClaim();
        const field = document.getElementById('fp-store-fingerprint');
        if (e.detail && e.detail.parameters && field) e.detail.parameters['store_fingerprint'] = field.value;
    });
    ...
    let _infrastructureAbortController = null;
    let _infrastructureRequestSerial = 0;
    let _infrastructureLastBbox = '';
    let _infrastructureFetchTimer = null;
    let _infrastructureZoomNoticeShown = false;

    function _infrastructureStyle(feature) {
        ...
        if (props.kind === 'rail') {
            return {
                ...
                opacity: 0.9,
                dashArray: props.historic ? '8 7' : null,
                lineCap: 'round',
                lineJoin: 'round',
            };
        }
        return {
            ...
            opacity: 0.9,
            dashArray: props.historic ? '4 7' : null,
            lineCap: 'round',
            lineJoin: 'round',
        };
    }

    function _bindInfrastructureFeature(feature, layer) {
        const props = feature?.properties || {};
        ...
        const osmUrl = typeof props.osm_url === 'string' && props.osm_url.startsWith('https://www.openstreetmap.org/')
            ? props.osm_url
            : '';
    ...
    function _setInfrastructureLoading(loading) {
    ...
    function _loadInfrastructure() {
        ...
        .then(data => {
            ...
            }).addTo(_infrastructureLayerGroup);
    ...
    function _scheduleInfrastructureFetch() {
        if (!_infrastructureLayerActive) return;
        clearTimeout(_infrastructureFetchTimer);
    ...
    function _renderMapAttribution() {
        const el = document.getElementById('page-footer-attribution-text');
        if (!el) return;
        el.textContent = _mapBaseAttributionText
            + (_infrastructureLayerActive ? ' · Infrastructure © OpenStreetMap contributors' : '');
    }
        ...
        document.querySelectorAll('.popup-stars').forEach(function(starsEl) {
            ...
            stars.forEach(function(star) {
                star.addEventListener('click', function() {
                    ...
                    // ulSendJson throws on a non-2xx; a bare fetch().then(json)
                    // reported "Rating saved" for ratings the server refused.
                    window.ulSendJson(`/dashboard/rest/reviews/create_or_update/${pinId}/`, 'PATCH', { rating: val })
                    .then(() => {
                ...
                document.querySelectorAll('.pin-popup .add-tag-button').forEach(function (button) {
                    button.addEventListener('click', function (e) {
                        ...
                        function saveInputValue() {
                            ...
                            window.ulSendJson(`/dashboard/rest/pins/${pinUuid}/`, 'PATCH', data)
                            .then(() => {
    ...
    map.on('contextmenu', function(e) {
        ...
        window.MapContextMenu.show({
            ...
            onClose: function () { _clearContextMenuMarker(); },
        });
    });
    map.invalidateSize();
    ...
    function _buildPlacesMarker(place) {
        ...
        marker.on('click', function (e) {
            ...
            const menu = window.MapContextMenu.show({
                lat: lat,
                lng: lng,
                zoom: map.getZoom(),
                clientX: anchor.x,
                ...
                extraItems: [
                    { icon: 'add_location', label: 'Add to my map', onClick: function () { openAddPinDialog(lat, lng, { defaultName: place.name, placeId: place.place_id }); } },
                    { icon: 'info', label: 'More information', onClick: function () { _showPlaceInfoPanel(place); } },
                ],
            });
    ...
    // wired per marker.
    function closePinPopupMenus() {
        document.querySelectorAll('.popup-menu-items:not([hidden])').forEach(function (menu) {
            menu.hidden = true;
    ...
    function togglePinPopupMenu(button) {
        const menu = button.parentElement?.querySelector('.popup-menu-items');
        if (!menu) return;
    ...
    document.addEventListener('click', function (event) {
        if (!event.target.closest('.popup-menu')) closePinPopupMenus();
    });
    ...
    async function promoteChildPins(pinSlug, childCount) {
        ...
        .catch(function () { toastr.error('Failed to promote child pins.'); });
    }
    ...
    function _renderBulkSelectToolbar() {
        ...
        window.ulBulkToolbar.sync('pins', n, {
            ...
            export: n >= 1 ? function () { document.getElementById('bulk-export-dialog').showModal(); } : null,
    ...
    function _exportSelection(format) {
        const uuids = Array.from(_selectedPinUuids);
        if (!uuids.length) return;
        document.getElementById('bulk-export-format-field').value = format;
        ...
        document.getElementById('bulk-export-dialog').close();
    }
    ...
    function _submitBulkDelete(uuids) {
        const confirmBtn = document.getElementById('bulk-delete-confirm-btn');
        confirmBtn.disabled = true;
        // ulSendJson carries the server's own refusal text into the catch below;
        ...
        .catch(function (err) { toastr.error((err && err.message) || 'Failed to delete pins.'); confirmBtn.disabled = false; });
    }
    ...
    // Same UX as the trip page's map<->list hover-sync (window.tripHighlightMarker
    // / tripHighlightActivity in trips/detail.html): the target pin is enlarged
    // and highlighted on the map so it's unmistakable, and hovering either a
    // marker or its mini-card in the dialog highlights the other, both ways.
    let _mergeTargetHighlightUuid = null;

    function _mergeDialogIsOpen() {
        const dlg = document.getElementById('bulk-merge-dialog');
        return !!(dlg && dlg.open);
    }

    function _setMergeTargetHighlight(uuid) {
    ...
    function _mergeHoverMiniCard(uuid, on) {
    ...
    document.getElementById('bulk-merge-dialog').addEventListener('mouseover', function (e) {
        const card = e.target.closest('.cat-merge-mini-card');
    ...
    document.getElementById('bulk-merge-dialog').addEventListener('close', function () {
        ...
        _selectedPinUuids.forEach(function (uuid) { _anyMarker(uuid)?.getElement()?.classList.remove('is-merge-hover'); });
    });

    // Marker DOM elements are recreated whenever the cluster layer regroups
    // (zoom, spiderfy), dropping the .is-merge-target class - re-stamp it,
    // same treatment _syncSelectionClasses gives .is-selected above.
    function _syncMergeTargetClass() {
    ...
    map.on('zoomend moveend', function () { requestAnimationFrame(_syncMergeTargetClass); });
    ...
    document.getElementById('bulk-merge-confirm-btn').addEventListener('click', function () {
        ...
        .catch(function (err) { toastr.error((err && err.message) || 'Merge failed.'); btn.disabled = false; btn.innerHTML = saved; });
    });
    ...
    // installed globally by core.js) - the same component the saved-filter pages
    // used to duplicate.
    function _makeLabelChipPicker(chipsElId, searchElId, suggElId, tabsElId) {
        return window.UrbanLensLabelPicker.createChipPicker({
            chipsEl: document.getElementById(chipsElId),
            searchEl: document.getElementById(searchElId),
            suggEl: document.getElementById(suggElId),
            tabsEl: tabsElId ? document.getElementById(tabsElId) : undefined,
        });
    }
    ...
    function openBulkEditDialog() {
        ...
        document.getElementById('bulk-edit-rating-value').value = '';
    ...
    document.getElementById('bulk-edit-confirm-btn').addEventListener('click', function () {
        ...
        const ratingValue = document.getElementById('bulk-edit-rating-value').value;
        if (ratingValue !== '') payload.rating = parseInt(ratingValue, 10);
        ...
        .catch(function (err) { toastr.error((err && err.message) || 'Update failed.'); })
    ...
    function createListAndAddPins() {
        ...
        // ulSendJson throws on a non-2xx carrying the server's own text, which is
        // what this endpoint sends: a duplicate list name answers 409 with
        // "You already have a list with that name." The previous version called
        ...
        window.ulSendJson(MAP_CFG.urls["listsCreate"], 'POST', { name }, { headers: { Accept: 'application/json' } })
            ...
            .catch((err) => {
                if (typeof toastr !== 'undefined') toastr.error((err && err.message) || 'Could not create that list.');
            });
    }
    ...
    function resetFilters() {
        ...
        if (typeof _dateBuiltRange !== 'undefined') _dateBuiltRange.sync();
        if (typeof _dateAbandonedRange !== 'undefined') _dateAbandonedRange.sync();
        if (typeof _lastViewedRange !== 'undefined') _lastViewedRange.sync();
    ...
    function applySavedFilter(chipEl) {
        ...
        const sliderFields = {
            ...
            min_detail_pins: 'min', max_detail_pins: 'max',
        };
        ...
        for (const key of [
            ...
            'date_built_after', 'date_built_before', 'date_abandoned_after', 'date_abandoned_before',
        ...
        for (const key of [
            'has_links', 'security_fences', 'security_alarms', 'security_cameras', 'security_security',
            'security_signs', 'security_vps', 'security_plywood', 'security_locked',
        ]) {
            if (!(key in criteria) || !criteria[key]) continue;
            const select = form.querySelector(`select[name="${key}"]`);
            if (select && !(select.value || '').trim()) select.value = criteria[key];
        }
    ...
    document.getElementById('filter-panel').addEventListener('pointerdown', function (e) {
        e.stopPropagation();
    });
    ...
    document.getElementById('pin-list-panel').addEventListener('pointerdown', function (e) {
        e.stopPropagation();
    });
    ...
    // three more times.
    function _wireDateRangeReset(afterId, beforeId, btnId) {
        function sync() {
    ...
    // ts/shared/location-search-engine.ts) bound to the {% map_search_bar %}
            ...
            marker.on('click', (function (mlat, mlng, mname) {
                return function (e) {
                    ...
                    window.MapContextMenu.show({
                        ...
                        clientY: anchor.y,
                        extraItems: [
                            { icon: 'add_location', label: 'Create pin', onClick: function () { openAddPinDialog(mlat, mlng, { defaultName: mname }); } },
                            { icon: 'close', label: 'Dismiss marker', onClick: function () { marker.remove(); } },
                        ],
                    });
                };
    ...
    // Backed by the shared picker engine (ts/shared/label-picker.ts, installed
    // globally by core.js) - the same engine drives the saved-filter dialog and
    // detail page, so include/exclude, chip dragging, the AND/OR toggle, and
    // the formula bar behave identically everywhere labels filter pins.
    // Serializes into the #fp-label-groups hidden input as label_groups JSON.
    const _labelPicker = (function () {
        ...
        return window.UrbanLensLabelPicker.createFilterPicker({
            els: {
                list: list,
                ...
                groupsInput: document.getElementById('fp-label-groups'),
                formulaBar: document.getElementById('fp-label-formula'),
            ...
            onChange: function () { htmx.trigger(document.getElementById('filter-form'), 'change'); },
        });
    })();
    ...
    var _iconGridRequest = null;

    function _parseIconCatalogue(html) {
        var holder = document.createElement('template');
        holder.innerHTML = html;
        var tabs = holder.content.querySelector('[data-icon-picker-tabs]');
        var items = holder.content.querySelector('[data-icon-picker-items]');
        return { tabs: tabs ? tabs.innerHTML : '', items: items ? items.innerHTML : '' };
    }

    function _loadIconCatalogue(url) {
        if (!_iconGridRequest) {
    ...
    function _iconGridStatus(grid) {
        var node = grid.querySelector('.icon-picker-status');
        if (!node) {
    ...
    function _fillIconGrid(id) {
        ...
        var status = _iconGridStatus(grid);
        status.textContent = 'Loading icons...';
        return _loadIconCatalogue(grid.dataset.gridUrl).then(function(catalogue) {
            ...
            var tabs = document.getElementById('icon-tabs-' + id);
    ...
    function _reapplyIconFilter(id) {
        var panel = document.getElementById('icon-panel-' + id);
        var search = panel ? panel.querySelector('.icon-picker-search-input') : null;
        var query = search ? search.value.trim() : '';
        if (query) {
            IconPicker.search(id, query);
            return;
        }
        var activeTab = panel ? panel.querySelector('.icon-tab.active') : null;
    ...
    var IconPicker = {
        toggle: function(id) {
            ...
            if (isHidden) {
                ...
                _fillIconGrid(id).then(function() { _reapplyIconFilter(id); });
            }
        },
    ...
    function _apdlgKindOk(label) {
        return !_apdlgActiveKind || label.kind === _apdlgActiveKind;
    }

    document.querySelectorAll('#apdlg-label-kind-tabs .tad-tab').forEach((tab) => {
        tab.addEventListener('click', () => {
            ...
            const searchEl = document.getElementById('apdlg-label-search');
            searchEl.focus();
        ...
        labelInput.addEventListener('keydown', function(e) {
            ...
            if (e.key === 'Tab' && items.length) {
                ...
                const topMatch = _apdlgAllLabels.find(b =>
                    !_apdlgSelectedLabels.find(s => s.id === b.id) && _apdlgKindOk(b) && b.name.toLowerCase().includes(q.toLowerCase())
                );
            ...
            if ((e.key === ',' || e.key === ' ') && items.length) {
                ...
                if (!wouldContinue && items.length === 1) {
                    e.preventDefault();
                    const topMatch = _apdlgAllLabels.find(b =>
                        !_apdlgSelectedLabels.find(s => s.id === b.id) && _apdlgKindOk(b) && b.name.toLowerCase().includes(q)
                    );
    ...
    function _refreshPinInStore(pinSlug, onDone) {
        ...
        .then(r => r.ok ? r.json() : null)
        .then(d => { if (d && d.pin) { _mergeLabels(d.labels); window.updateCachedPin(d.pin); } onDone?.(); })
        .catch(() => {
            window.invalidatePinCache();
