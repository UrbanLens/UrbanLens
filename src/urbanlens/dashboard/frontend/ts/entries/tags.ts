import { BulkEntityManager, type BulkEntityManagerConfig } from "../shared/badge-bulk-manager";
import { installGlobalIconPicker } from "../shared/icon-picker";
import { installGlobalColorPicker } from "../shared/color-picker";
import { installGlobalParentSearch } from "../shared/parent-search";

installGlobalIconPicker();
installGlobalColorPicker();
installGlobalParentSearch();

/** Live preview for the "upload custom icon" file input on the new-tag form. */
function showNewCustomPreview(input: HTMLInputElement): void {
    const file = input.files?.[0];
    if (!file) return;
    const preview = document.getElementById("new-custom-preview") as HTMLImageElement | null;
    if (!preview) return;
...
declare global {
    interface Window {
        showNewCustomPreview: typeof showNewCustomPreview;
    }
}

function resetCustomIconPreview(): void {
    const preview = document.getElementById("new-custom-preview") as HTMLImageElement | null;
    if (!preview) return;
    preview.src = "";
...
function init(): void {
    ...
    const config: BulkEntityManagerConfig = {
    ...
    document.getElementById("new-custom-input")?.addEventListener("change", (e) => {
        showNewCustomPreview(e.target as HTMLInputElement);
    });
}
