/**
 * "Core" globals bundle: LocationSearchEngine + MarkupEngine.
 *
 ...
 * `MarkupEngine.createDrawSession(...)` synchronously as soon as they run -
 * not inside a DOMContentLoaded handler. `type="module"` scripts are always
 * deferred until after the document has finished parsing, which would run
 * this *after* those classic scripts and leave the globals undefined when
 * they're needed. Loading as a blocking classic script preserves the exact
 * head-executes-before-body ordering the site already depends on.
 */
