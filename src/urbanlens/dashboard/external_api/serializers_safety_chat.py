...
It is deliberately absent from every payload in ``serializers.py`` (see
:class:`~urbanlens.dashboard.external_api.serializers.SafetyCheckinContactSerializer`) and must stay
absent here. :class:`SafetyCheckinMessageSerializer` touches a contact row directly, so it exposes
only that contact's *display name*, never the row itself.
Emitting a username or profile uuid for them is not merely null-shaped noise: it would invite a
client to treat "contact" and "user" as the same kind of thing and, for instance, link a profile
...
# The model types below are imported at runtime, not under TYPE_CHECKING, even though they appear only in
# annotations. drf-spectacular introspects every ``SerializerMethodField`` handler with
# ``typing.get_type_hints`` to infer the field's schema type, and that call *evaluates the whole signature* -
# including the ``obj`` parameter it has no interest in.
...
class SafetyCheckinMessageSerializer(serializers.Serializer):
    ...
    ``is_mine`` is resolved against a ``viewer`` profile passed in the serializer context.
    It is computed server-side rather than left to the client because a client comparing usernames would
    get it wrong for exactly the participants that matter: two different people can appear under the
    same display name (an email-only contact's ``name`` is free text the owner typed, and nothing stops
    it matching another participant's username), so the only reliable identity comparison is on the
    sender's profile id, which is not in the payload and should not be.
    """
    ...
    def get_sender_kind(self, obj: SafetyCheckinMessage) -> str:
        ...
        Returns:
            ``"profile"`` for the owner or a partner, ``"contact"`` for an emergency contact posting through
            their portal link, and ``"unknown"``...
            defensive padding: both sender foreign keys are ``SET_NULL``, so a deleted account or a removed
            contact leaves its messages in place with no sender at all,...
        """
    ...
    def get_is_mine(self, obj: SafetyCheckinMessage) -> bool:
        ...
        Returns:
            True when the message's sending profile is the viewer.
        """
...
class SafetyCheckinMessageCreateSerializer(serializers.Serializer):
    ...
    The bounds mirror ``services.visits.safety.create_chat_message`` rather than replacing it.
    ``MAX_CHAT_MESSAGE_LENGTH`` is imported rather than restated so the two cannot drift into
...
class SafetyPartnerRoleSerializer(serializers.Serializer):
    ...
    It carries the check-in's ``title`` even though the caller may not have accepted yet, because that
    is already disclosed: the invitation notification and email both name the check-in
    (``services.visits.safety._notify_checkin_partner_invite``).
    Withholding it here would only leave the client rendering "someone invited you to something".
    """
    ...
    def get_checkin_uuid(self, obj: SafetyCheckinPartner) -> str:
        ...
        Returns:
            ...
            decline endpoints. Uuid, not slug: a slug is unique only per owner, so it cannot address another
            person's check-in.
        """
    ...
    def get_checkin_title(self, obj: SafetyCheckinPartner) -> str:
        ...
        Returns:
            The title, which is blank for a check-in whose PII has already been scrubbed into its encrypted
            archive.
        """
    ...
    def get_checkin_status(self, obj: SafetyCheckinPartner) -> str:
        ...
        Returns:
            The status value, so a client can tell a still-live invitation from one whose check-in already
            concluded while it sat unanswered.
        """
    ...
    def get_invited_by_username(self, obj: SafetyCheckinPartner) -> str | None:
        ...
        Returns:
            The inviter's username, or None if that account is gone.
        """
...
class _PartneredCheckinOwnerFieldsMixin:
    ...
    Every owner-facing check-in serializer omits the owner, because there the owner is always the
    caller.
    On the partner surface the owner is the single most important field in the payload - the whole row
    means "this person is out somewhere" - so both the summary and the detail shape gain it, from one
...
class PartneredCheckinSummarySerializer(_PartneredCheckinOwnerFieldsMixin, SafetyCheckinSummarySerializer):
    ...
    Identical to the owner's own list row plus the owner's identity - a partner watching several people
    needs to tell the rows apart, and deliberately no thinner than the owner's row otherwise, because an
    accepted partner already has the full detail view on the website.
    """
...
class PartneredCheckinDetailSerializer(_PartneredCheckinOwnerFieldsMixin, SafetyCheckinDetailSerializer):
    ...
    Serving a thinner payload here would mean the mobile client showed a watcher less than the website
