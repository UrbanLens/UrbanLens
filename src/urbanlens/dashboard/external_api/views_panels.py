...
Exposes exactly the panels that have opted in via a non-empty ``PanelSource.api_kinds`` - see
``services.pins.external_data`` for why that, rather than whether ``api_payload`` happens to return
data right now, is the authoritative "exposed to the API" signal.
The satellite and street-view carousels are deliberately not among them: their web payload is base64
``data:`` URIs (5-15MB/response), and this API's throttle counts requests, not bytes, so exposing
them would let one throttled-by-count credential become an unmetered bandwidth amplifier.
"""
...
#: Retry hint when a fetch could not be scheduled (unknown-to-scheduler, suppressed after a recent failure, or
#: the Celery broker was unreachable) - deliberately longer than the in-flight case, since none of those resolve
...
class PinPanelsListView(OwnedPinMixin, ExternalApiView):
    ...
    Only panels with non-empty ``api_kinds`` are listed.
    A source gated by ``gate(pin)`` (e.g. no coordinates to work with) or by a subscription feature the
    caller's account lacks is omitted entirely rather than merely marked unready - the same rule the web
    tab strip applies (``controllers.pin._viewer_may_see_panel``, backed by the same
    ``panel_visible_to``).
    """
...
class PinPanelDetailView(OwnedPinMixin, ExternalApiView):
    ...
    def get(self, request: Request, pin_slug: str, panel_key: str) -> Response:
        ...
        if source is None or not source.api_kinds or not gate_allows(source, pin) or not panel_visible_to(request.user, source):
            # Same anti-enumeration policy as the rest of this API: a feature-gated panel the caller can't see
        ...
        if source.is_ready(pin):
            ...
            # results); the JSON equivalent of the web panel's quiet 204.
