...
The write side is a genuinely different contract - every field optional, at least one required
overall - so it is a separate class rather than a flag on that one.
Those shapes are declared here so the published schema describes them rather than leaving a native
client to discover them from a sample response.
"""
...
class TripPermissionsUpdateSerializer(serializers.Serializer):
    ...
    A ``default=`` on any of these would silently reintroduce the very defect the service was fixed for

    - a client toggling one switch rewriting three other permissions on a trip it shares with other
      people - and would do so invisibly, because the request would s...
    """
    ...
    def validate(self, attrs: dict[str, Any]) -> dict[str, Any]:
        ...
        Raises:
            serializers.ValidationError: The body named none of the four permission fields.
        """
...
class TripCalendarStatusSerializer(serializers.Serializer):
    ...
    ``connected`` and ``linked`` are separate on purpose: a connected account with no link means the
    calendar is reachable but this trip has never been exported to it, which is a different next action
    for the client than having no account at all.
    ``account_email`` answers a question the older status block could not: a user with more than one
    Google account can otherwise see that a shared trip's itinerary is being written to *some* calendar
    without being able to tell which.
    """
...
class TripCalendarExportRequestSerializer(serializers.Serializer):
    ...
    ``auto_sync`` is presence-keyed like everything else on this surface: omitting it leaves whatever
    the existing link already had, so re-exporting a trip to refresh its dates cannot quietly switch off
    a user's ongoing sync.
    """
...
class TripCalendarRemovalResponseSerializer(serializers.Serializer):
    ...
    calendar = TripCalendarStatusSerializer(read_only=True)
    #: False when the trip was not on this caller's calendar to begin with. Deliberately not an error:
...
class TripCalendarBlockedSerializer(serializers.Serializer):
    ...
    Both cases this covers - no connected calendar, and a grant Google has since rejected - are fixed by
    the same one-time consent flow, which cannot happen over an API credential: it ends in a redirect
    back to a callback that needs a browser session to identify the user.
    ``error_code`` is machine-readable because the two cases warrant different prose ("connect your
    calendar" versus "your connection expired") but the same action, and a client should branch on the
    ...
    #: The **site's own** connect route, to be opened in a system browser. Never a Google authorization URL: one
    #: minted by this API would return to the OAuth callback with no session attached, bounce to the login page,
