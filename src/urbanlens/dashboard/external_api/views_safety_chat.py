...
Everything partner-facing is reached by **uuid only**: a check-in's slug is unique per owner, not
globally (``controllers.safety._get_checkin_as_partner`` has to swallow ``MultipleObjectsReturned``
for exactly this reason), so a slug cannot safely name another person's check-in. **404, never 403,
everywhere.** A 403 on a check-in identifier confirms that the identifier names a real, live
check-in belonging to *someone* - i.e. that a specific person is out somewhere right now

- which is a disclosure even with no payload attached.

- **Chat had no REST endpoint.** The only non-session chat surface was the tokenized contact-portal
  WebSocket, which authenticates a *contact*, not the owner a...
- **An invited partner could not answer.** Invites could be sent through the API but only accepted
  in a browser, so the API could create a state it had no way ...
- **A partner could not mark the owner safe.** The single most useful thing a watcher does - "I
  found them, stand everyone down" - was web-only, while the esca...
"""
...
class SafetyCheckinMessagesView(SafetyCheckinViewerScopedView, PaginatedListMixin):
    ...
    rather than the owner-scoped base, because the chat's whole point is that the owner and their
    accepted partners are talking to *each other*: an owner-scoped lookup would give the explorer a
    monologue and give partners nothing at all.
    A contact's access to a check-in is the tokenized portal, which only opens once a check-in
    escalates; routing them through this endpoint would hand a contact early access to a conversation
    they are not yet part of, on the strength of a credential the owner never granted them.
    """
    ...
    def get(self, request: Request, checkin_slug: str) -> Response:
        ...
        Returns:
            A paginated transcript, or a 404 if the caller may not watch this check-in - including when it
            does not exist.
        """
        ...
        messages = (
            ...
            # Newest first, and *total*: `created` alone ties whenever two messages land in the same instant -
            # which is exactly what happens when a system message is written alongside a user's, as mark-safe
            # and partner-accept both do - and an unstable sort silently drops or repeats rows across page
            # boundaries.
            .order_by("-created", "-pk")
        )
        # The viewer goes in the context rather than being compared client-side: see
        # SafetyCheckinMessageSerializer.is_mine for why display names are not a safe identity comparison.
    ...
    def post(self, request: Request, checkin_slug: str) -> Response:
        ...
        A REST-sent message has to reach the live conversation exactly as a web-sent one does, and the only
        way to guarantee that is to not have a second send path at all.
        ...
        Returns:
            201 with the created message, 400 for a blank or over-long body, 409 once the check-in has been
            archived, or 404 if the caller may not...
        """
        checkin = self.get_viewable_checkin(request, checkin_slug)
        if checkin is None:
            return self.not_found()
        ...
        try:
            # contact=None: this endpoint authenticates an account, never a portal token, so the sender is
            # resolved to the caller's own profile by resolve_message_sender.
            ...
            # A state conflict, not a malformed request: the body was fine, the check-in's plaintext is simply
            # already sealed into its encrypted archive.
...
class SafetyPartnerScopedView(ExternalApiView):
    ...
    The partner surface asks "are you an accepted partner on someone *else's* check-in", and it must
    exclude the owner: an owner reaching a partner-only write would be marking themselves safe through
    an endpoint whose entire audit meaning is "somebody else confirmed they are alright".
    """
    ...
    def invite_not_found(self) -> Response:
        ...
        In particular it must not become "you already declined this", which would confirm the check-in
        exists to anyone who guessed a uuid.
    ...
    def checkin_not_found(self) -> Response:
        ...
        Byte-identical to the owner- and viewer-scoped bases' 404 so that a client probing across surfaces
        cannot learn which one rejected it.
    ...
    def partnered_detail_response(self, checkin: SafetyCheckin, *, status: int = 200) -> Response:
        ...
        Re-fetching rather than serializing the passed instance is what makes this correct after a write: a
        just-resolved check-in handed straight to the serializer would still carry the prefetch caches and
        the status the request arrived with.
...
class SafetyPartnerInvitesView(SafetyPartnerScopedView, PaginatedListMixin):
    ...
    Not a convenience listing - it is the only way the accept and decline endpoints are reachable at
    all.
    Without this endpoint an API-only client can be invited and then have no route to say yes.
    """
    ...
    def get(self, request: Request) -> Response:
        ...
        Returns:
            ...
            Only ever the caller's own: the queryset is scoped to their profile, so this cannot be used to
            enumerate anyone else's invitations.
        """
...
class SafetyPartnerInviteAcceptView(SafetyPartnerScopedView):
    ...
    def post(self, request: Request, checkin_uuid: uuid_module.UUID) -> Response:
        ...
        Idempotent by design, and 200 rather than an error on a repeat.
        ...
        Returns:
            200 with the caller's partner row, now ACCEPTED, or 404 if they hold no invitation for that
            check-in.
        """
        ...
        # Re-read rather than serializing the in-memory row.
...
class SafetyPartnerInviteDeclineView(SafetyPartnerScopedView):
    ...
    def post(self, request: Request, checkin_uuid: uuid_module.UUID) -> Response:
        ...
        Repeating this is a 404, not another 204, and that is the honest answer rather than a wart:
        declining *deletes* the row, so on the second call there is genuinely no invitation to act on.
        ...
        Returns:
            204 on success, or 404 if the caller holds no partner row for that check-in.
        """
...
class SafetyPartnerCheckinsView(SafetyPartnerScopedView, PaginatedListMixin):
    ...
    def get(self, request: Request) -> Response:
        ...
        Returns:
            A paginated list of other people's check-ins where the caller holds an ACCEPTED partner row,
...
class SafetyPartnerCheckinDetailView(SafetyPartnerScopedView):
    ...
    def get(self, request: Request, checkin_uuid: uuid_module.UUID) -> Response:
        ...
        Returns:
            200 with the detail document, or 404 when the caller is not an accepted partner on it -
            identical to the response for a uuid that names...
        """
...
class SafetyPartnerMarkSafeView(SafetyPartnerScopedView):
    ...
    def post(self, request: Request, checkin_uuid: uuid_module.UUID) -> Response:
        ...
        The write this whole surface exists for: it stops the escalation beat from emailing the owner's
        emergency contacts, and it does so from a phone, which is where a partner actually is when they find
        someone.
        ...
        Returns:
            200 with the refreshed detail document, 409 if the check-in has already been resolved by someone
            else, or 404 if the caller is not an...
        """
        caller = self.resolve_caller(request)
        ...
        if checkin.is_resolved:
            # An up-front courtesy check only. The service resolves through a conditional UPDATE, so a contact
            # and a partner racing to report the same person safe cannot both win regardless of what this sees.
