...
Mirrors ``dashboard/controllers/pin_bulk.py`` (the main map's multi-select toolbar), with two
deliberate departures from that internal surface, both made for consistency with the rest of this
API rather than with the page the logic was copied from:

- Every optional field here follows the same "absent means untouched, an explicit null clears it"
  rule :class:`~urbanlens.dashboard.external_api.serializers.Pi...
- ``add_label_uuids``/``remove_label_uuids`` uuids must resolve to a label the caller may use, or
  the whole request is refused with 400 - the internal view sil...
"""
...
class PinBulkDeleteResponseSerializer(serializers.Serializer):
    ...
    ``undo_uuid`` restores every deleted pin (and any child pins swept up with it) via the existing
    generic undo surface - ``POST undo/{undo_uuid}/restore/`` - rather than a bespoke restore endpoint
    of its own.
    """
...
class PinBulkEditSerializer(serializers.Serializer):
    ...
    ``rating`` writes/clears a :class:`~urbanlens.dashboard.models.reviews.model.Review` for the caller
    against each selected pin, the same underlying write ``PUT``/``DELETE`` ``pins/{pin_slug}/review/``
    makes one pin at a time.
    """

    uuids = serializers.ListField(child=serializers.UUIDField(), min_length=1, max_length=500)
    description = serializers.CharField(required=False, allow_null=True, allow_blank=True)
    rating = serializers.IntegerField(required=False, allow_null=True, min_value=1, max_value=5)
    #: Delta, not a replacement: labels not named here are left alone on every selected pin. Contrast
    #: ``PinUpdateSerializer.label_uuids``, which replaces one pin's complete set - a full-replacement doesn't
    #: generalize across pins that don't already share the same labels.
