...
Resolving through ``OwnedPinMixin`` is therefore not defensive tidiness; skipping it would be a
cross-tenant write.
"""
...
class PinWikiSyncApiView(OwnedPinRequiredMixin, ExternalApiView):
    ...
    #: Stacked, not substituted: the standard burst/read/write caps still apply, and this adds the much tighter
    #: cap these two endpoints need because their cost is unbounded in the caller's own data rather than in
    #: request count.
...
class PinWikiSyncPushView(PinWikiSyncApiView):
    ...
    Requires **both** ``pins:write`` and ``wiki:write``: the call reads the caller's private child pins
    and writes public child wikis plus an attributed ``WikiEdit`` row, so consenting to only one half is
    not consent to this.
    ``HasApiKeyScope`` requires every declared scope, so the frozenset below is a genuine AND.
    """
    ...
    def post(self, request: Request, pin_slug: str) -> Response:
        ...
        Returns:
            200 with how many child wikis were created, and whether the property has a wiki at all.
        """
        ...
        # Checked up front so the expensive matching pass is skipped entirely for a property with no wiki - the
        # service would do all that work and then return 0 anyway, and the client cannot tell the two zeros
        # apart without this flag.
        ...
        if not children:
            # A stale selection (the child pins were deleted or re-parented since the client last looked) is a
            # no-op, not an error - same as the internal toolbar, which shows an informational toast.
...
class PinWikiSyncPullView(PinWikiSyncApiView):
    ...
    Requires ``pins:write`` (it creates pins) and ``wiki:read`` (it reads the community page's markers)

    - the inverse split from the push endpoint, and the reason both scopes are named per direction
      rather than one "sync" scope.
    """
    ...
    def post(self, request: Request, pin_slug: str) -> Response:
        ...
        Returns:
            200 with how many child pins were created, and whether the property has a wiki at all.
        """
        pin = self.get_owned_pin_or_404(request, pin_slug)
        if Wiki.objects.get_for_location(pin.location) is None:
            return self.result(0, wiki_exists=False)
