...
These games exist to get real data into wikis, so their write paths have side effects on community
content - which is exactly why they are routed as their own domain rather than being folded into
``urls_wiki_community``: a client's game scope should not imply the ability to edit a wiki directly.
Sessions are stateful and time-bounded (a stalled session is abandoned rather than left open
forever), so endpoints here are session-scoped - ``games/<...>/sessions/<...>/`` - and never trust a
...
#: Routes contributed by this domain. SpotGuessr only, and solo-play only: the lobby/invite/join/begin/end/chat
#: half of the game is deliberately not routed here (see ``views_games`` for why mirroring it would strand a
#: client mid-round).
