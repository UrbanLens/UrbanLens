...
class SpotGuessrRoundImageView(SoloSessionOnlyMixin, ExternalApiView):
    ...
    The stored file is served as-is, through the same hand-off as the media gate: the upload
    pipeline re-encodes every photo from its pixels, so it carries no GPS or other metadata
    naming the answer. Requires both ``games:read`` (this is game content) and ``media:read``
    (these are the user's own photo bytes) - ``HasApiKeyScope`` requires every declared scope, so
    a credential holding only one of the pair gets nothing.

    Charged against the media budget rather than the API read budget, for the same reason the
    media gate is: a screen of images is a burst of file requests, and metering them against the
    JSON caps would refuse legitimate play while still leaving a leaked key able to pull files
    all day.
    """
