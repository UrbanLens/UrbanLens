...
The friendship shapes - ``FriendshipSerializer`` and friends - predate the per-domain split and stay
in ``serializers.py``; the unblock endpoint reuses ``FriendshipSerializer`` rather than defining a
near-identical twin, so a client parses one relationship shape everywhere.

- **Write serializers validate; they do not decide.** The annotation bounds restated here (nickname
  length, trust range) mirror ``services.profile.profile_anno...
- **Nothing in this module is a read model for someone else's data.** The annotation payload
  describes what the *caller* recorded about a subject. If a field i...
"""
...
#: Mirrors ``forms.profile_form._DISCORD_HANDLE_RE`` /
#: ``services.profile.profile_settings._DISCORD_USERNAME_RE`` - this is the public-social-link Discord entry
#: (``SocialLink(platform="discord")``), a distinct row from the private ``Profile.discord_username`` contact
...
class AvatarEmojiSerializer(serializers.Serializer):
    ...
    Both fields are constrained to closed sets.

    - an arbitrary string there would be markup injection into a file the site subsequently serves to
      every viewer of that profile.
    """
    ...
    def validate_color(self, value: str) -> str:
        ...
        Accepts any case (``#4caf50`` and ``#4CAF50`` both work) because hex colours are routinely
        lowercased by client-side pickers, and returns the enum's own spelling so the stored SVG is
        byte-identical whichever form arrived.
...
class ProfileAnnotationsSerializer(serializers.Serializer):
    ...
    The subject of an annotation can never read it - asking for your own annotations returns nulls, not
    the opinions other people hold about you.
    ``note_count`` rather than the notes themselves: notes are an unbounded collection with their own
    endpoint (``/profiles/{slug}/notes/``), and this payload is fetched every time a profile screen
    opens.
    """
...
class ProfileNicknameWriteSerializer(serializers.Serializer):
    ...
    ``allow_blank`` is off deliberately.
    Blank is how the site's HTMX widget signals "clear this", but an API with a real ``DELETE`` on the
    same route does not need a second, ambiguous way to say it - and accepting blank here would make
    ``PUT`` sometimes a delete, which no client would expect.
    """
...
class ProfileTrustWriteSerializer(serializers.Serializer):
    ...
    Bounded to the same range the model's validators enforce.
    The bound is checked here as well as in the service because ``update_or_create`` does not run field
    validators - without one of the two, an out-of-range rating would be stored and only surface as a
    rendering oddity much later.
    """
...
class SocialLinkWriteSerializer(serializers.Serializer):
    ...
    ``handle`` means different things per platform: a username for the eight handle-based platforms -
    validated by the same ``services.profile.social_links.validate_handle`` rules a pasted URL is
    checked against internally, so a directly-submitted handle can never be looser than one the web UI
    would have extracted - Discord's own free-form username (it has no public profile URL to parse one
    from), or the complete URL itself for ``website``.
    """
    ...
    def validate(self, attrs: dict) -> dict:
        ...
        Returns:
            *attrs*, with ``handle`` normalized (stripped of a leading ``@`` for username platforms;
            stripped of its fragment for ``website``).

        Raises:
            serializers.ValidationError: The handle is blank, or fails its platform's rule.
        """
        ...
        if platform == "website":
            # Mirrors parse_social_link's own two-step check.
...
class SocialLinksReplaceSerializer(serializers.Serializer):
    ...
    PUT rather than PATCH, matching ``SafetyContactDefaultsSerializer``'s own precedent: the underlying
    write deletes and recreates the whole set, and there is no per-entry addressing (a platform key
    inside the body isn't one) to PATCH against.
    Submitting an empty ``links`` list clears every platform.
    """
    ...
    def validate_links(self, value: list[dict]) -> list[dict]:
        ...
        Raises:
            serializers.ValidationError: A platform appears more than once, which would make the outcome
