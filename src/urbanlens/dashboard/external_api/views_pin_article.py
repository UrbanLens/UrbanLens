...
A pin can carry an article exactly as a community wiki can - same ``Article`` row, same Markdown
pipeline, same append-only ``ArticleRevision`` history - but the two are completely different kinds
of content, and this module exists mostly to keep that difference from being lost.
**Scopes are ``pins:read``/``pins:write``, never ``wiki:*``.** A pin article is the owner's private
write-up of their own pin: route notes, access details, things they specifically did not publish to
the community page.
"""
...
class OwnedPinRequiredMixin(OwnedPinMixin):
    ...
    It lives here because that module was not in this change's scope.

    TODO: move ``get_owned_pin_or_404`` onto ``views.OwnedPinMixin`` and repoint this class and
    ``views_wiki._own_pin_or_404`` at it.
    """
    ...
    def get_owned_pin_or_404(self, request: Request, pin_slug: str) -> Pin:
        ...
        Returns:
            The caller's matching pin, with ``location`` and ``profile`` already joined.

        Raises:
            Http404: No such pin, or it belongs to someone else - deliberately the same response for both,
            so a pin slug cannot be used to test whether...
        """
...
class PinArticleApiView(OwnedPinRequiredMixin, ExternalApiView):
    ...
    :meth:`pin_article` is the module's whole authorization story, and it is deliberately shaped as a
    *lookup* rather than a check that could answer 403
...
class PinArticleView(PinArticleApiView):
    ...
    def put(self, request: Request, pin_slug: str) -> Response:
        ...
        Creates the article on first save - which is why this resolves the pin rather than the article, and
        is the escape hatch for the 404 GET returns on a pin that has never been written to.
        ...
        Returns:
            200 with the saved article, or 409 when someone else (the owner on another device, or another
            client holding the same key) saved in the...
        """
        pin = self.get_owned_pin_or_404(request, pin_slug)
        ...
        try:
            ...
            # Same contract as the wiki article endpoint and the internal editor: refuse the write and hand back
            # which revision is actually current, so the client can fetch it, show the difference, and re-base
...
class PinArticleRevisionDetailView(PinArticleApiView):
    ...
    def get(self, request: Request, pin_slug: str, revision_id: int) -> Response:
        ...
        # article= in the lookup, never a bare id: revision ids are sequential across every article in the
        # database, so an unscoped get would expose other users' private pin articles one integer at a time.
...
class PinArticleRevisionRestoreView(PinArticleApiView):
    ...
    def post(self, request: Request, pin_slug: str, revision_id: int) -> Response:
        ...
        History is append-only: nothing is deleted, the old content is written forward and tagged
        ``restored_from`` so the lineage stays visible.
