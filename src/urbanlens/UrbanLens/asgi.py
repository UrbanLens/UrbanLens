...
def _warm_urlconf() -> None:
    ...
    The imports below cover the *websocket* patterns only, so the HTTP URLconf -
    which reaches every controller, and through them GeoPandas/Shapely - stays
    unloaded until something asks for an HTTP route. Measured at 2.26s of CPU.

    daphne is a single process with ``cpus: 1``, and that import holds the GIL,
    so paying it on request one stalls the event loop serving every WebSocket on
    ...
    gunicorn config, so it needs its own.
    """
    try:
        logging.getLogger(__name__).info("URLconf warmed: %d root patterns", len(get_resolver().url_patterns))
    except Exception:
