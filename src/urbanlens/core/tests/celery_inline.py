...
Most tests want an enqueued task to stay enqueued: they assert a request returned quickly and
handed work off, and stub ``safely_enqueue_task`` to check that it did. A few are the opposite -
the thing under test only becomes true once the task runs:
...
With no worker draining the broker, those tasks are silently dropped and the assertion can never
hold - which reads as a bug in the feature rather than a missing runner.

The patch target is the subtle part. Every caller binds the name into its own module at import
(``from ... import safely_enqueue_task``), so patching where the function is *defined* replaces
an attribute none of them read: the real enqueue runs, the task goes to a broker with no worker,
and the test waits for something that will never arrive. When a `WebsocketCommunicator` does the
waiting, that surfaces as `TimeoutError` on a socket - reads as a slow host, not a broken fixture.

``CELERY_TASK_ALWAYS_EAGER`` would fix all of them at once and is what several of these tests'
own docstrings assume, but it is far too blunt in practice: turning it on runs *every* task a
test incidentally triggers, and merely creating a profile fans out into label enrichment that
then trips the outbound rate limiter. Selecting the one or two tasks a test actually depends on
keeps the blast radius to the behaviour being asserted.
"""
