...
**1. Do not edit this file.** A hook blocks it. Write what you learn in
`docs/` instead. If something truly belongs here, propose the exact wording
and let Jess apply it.

**2. With few exceptions, everything in `docs/` was written by a Claude agent, not by Jess.**
It is evidence, not authority. Re-measure before you rely on a figure, and
rewrite the doc when you find it wrong.

**Finding things:** docs/INDEX.md

One line per document, greppable. 

`P` problem (a known defect, and how it was found) · `I` idea (unvalidated) ·
`D` decision (we chose X over Y because Z) · `X` experiment (one measurement,
with its unit of analysis) · `T` task · `PL` plan · `R` reference (how a
subsystem works today). `N` note.

`grep -E '^\| (P7|T1) ' docs/INDEX.md` finds a record. IDs are allocated from
the index and never reused or renumbered.
...
`NOTES.md` - explains non-obvious behavior 
...
`METRICS.md` - the `/metrics` Prometheus endpoint.
...
`pyproject.toml`'s `extend-exclude` skips `tests`, `migrations` and `__init__.py`.

**MyPy**
The purpose of mypy is to find bugs and improve code quality, not to silence warnings. This can require fixing code or types at the origin of the call, rather than the point of failure. Never use "cast" or similar fixes; Fix bad assumptions, implement generics. If you're unsure, mark it as a TODO instead of silencing the warning.
...
Common commands should be added to `pyproject.toml` scripts, `package.json`, and/or VSCode tasks.
...
For complex tasks, set a wakeup for 2 minutes, then review the work you did with fresh eyes before finishing it, since
adversarial reviews often produce valuable input.
...
## Tech Stack
Django, channels (websockets), PostGIS, SCSS, TSX, bun, HTMX, oauth2, passkeys (webauthn) + TOTP 2FA, Celery, django-gis, geopandas, shapely, fastkml, geopy

## Code Quality
Prefer OOP, inheritance, and generics for abstraction and extensibility.

Use Google docstrings. Sphinx consumes them for ReadTheDocs-style documentation - completeness matters.

- Type hints throughout; MyPy with Django stubs
- Modern Python (3.12+)
- Choose actively maintained, modern libraries over dated equivalents

Comments should be concise, and only included when not obvious. Assume that someone competent will be working on code after you; unnecessary explanation is a burden.
...
- Any new pin/location share path must call `resolve_origin_share` + `record_share_exposure` to keep the `LocationExposure` provenance chain intact.
...
- Anything that hands user-uploaded bytes to a parser (Pillow, ffmpeg, LibreOffice, GDAL, `zipfile`, lxml) must be decorated `@untrusted_parse` and reached only from a task declaring `queue=SANDBOX_QUEUE`. Declare the queue on the task, never at the `apply_async` call site. See `docs/MEDIA_PIPELINE.md`.
