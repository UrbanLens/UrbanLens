/**
 * Configuration for a run, read once from the environment and validated eagerly. Everything the suite needs to know about *which* deployment it is pointed at lives here.
 *
...
/** A run identifier stamped into every record the suite creates. Anything left behind by an interrupted run is greppable by this, and the provisioning command's `--purge` can find it without guessing. */
...
export const env = {
    ...
    runId,
    /** Prefix on every name this suite writes, so leftovers are identifiable. Letters, digits and hyphens only. */
    ...
    /** Whether `/health/primary` should answer 200 (read-replicas legitimately answer 503). */
    ...
    /** Run the specs that wait on scheduled sweeps (`specs/slow/`): check-in escalation, message hard-delete. */
    runSlow: readBoolean("UL_E2E_SLOW", false),
    ...
    // Long: the API client waits out 429s, and a burst-limit wait can be a full minute.
