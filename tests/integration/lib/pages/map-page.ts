/** The map, which is the application's front door. Leaflet renders into a canvas of positioned tiles and markers, so almost nothing here is assertable by text. */
...
export class MapPage {
    ...
    /** Waits until Leaflet has actually taken over the container. `#map` exists in the HTML whether or not the script ran, so its presence proves nothing. */
    ...
    /** Fetches the map's own pin feed as the signed-in user. Uses the page's session rather than an API key, so this exercises the internal, session-authenticated endpoint the map itself calls. */
