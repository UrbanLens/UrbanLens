...
* ~~Provide explanation of how to do a google takeout to import pins. (Possible onboarding process?) [UL-142]~~ (pages/help/import_pins.html)
...
* One time: When filtering the map by rating, I saw a single pin without a rating. [UL-270]
...
* A few times: Quickly switching between map layers sometimes is weird. Foggy sat view, etc. (Foggy may have just been loading indicator?? Maybe that loading indicator didn't go away when it should? I'm not sure.) [UL-273]
* One time: After going to a suggested jump to point, clicking the temporary marker to create a pin, and submitting, the new pin doesn't show up on the map without a refresh. (maybe this was due to latency, which is a separate TODO item?) [UL-274]
...
* ~~Support partial cache updates, instead of refreshing the cache for all pins at once. [UL-22]~~ (MapPinCache upsert/delete per pin via signals)
* ~~Limit failed login attempts. [UL-28]~~ (login lockout via SiteSettings login_max_attempts/login_lockout_minutes)
* ~~Add metadata for emojis (i.e. icons) to aid in searching for them. [UL-12]~~ (keyword completeness tracked in UL-174)
...
* ~~Password reset should go to application page, not django password reset page. [UL-256]~~ (registration/password_reset_*.html extend the app auth theme)
...
* ~~In tools page, do only admins have access to the db backup tool? I guess we should probably move that to an admin-only page to be certain. (also, page width is wrong). [UL-282]~~ (AdminToolsView/BackupStartView use PermissionRequiredMixin)
...
* ~~Allow users to specify their signal username. Ideally, make it easy for users to add them to their signal contacts if it's provided. [UL-45]~~ (signal/whatsapp/telegram/matrix fields + contact_visibility)
...
* ~~When user uploads a photo containing GPS data and a date, mark a visit log entry for that date, and attach the photo to it. [UL-49]~~ (Memories > Photos organize + VisitSuggestions)
...
* ~~Request browser notification permissions for users. [UL-56]~~ (notification_preferences.html)
* ~~Integrate gotify (?) for notifications to site admin. [UL-57]~~ (services/notifications.py _send_gotify)
...
* ~~Allow importing of timeline data to mark pins as Visited [UL-118]~~ (google location-history/my-activity import)
...
* ~~Share button on pin details page to share with a specific friend. [UL-20]~~ (pin_sharing.py PinShareDialog/Create)
* ~~"Accept" / "Reject" shared pin for the user being shared with. [UL-21]~~ (PinShareRespondView)
* Implement "hide user", and "mute user" features, alongside the existing "block user" feature. [UL-27]
* ~~Proper CI/CD pipeline, tags, releases, etc. [UL-25]~~ (ci/security/publish/release-please workflows)
...
* ~~Automatic backups [UL-139]~~ (backup_frequency_hours/backup_retention + backup tooling)
...
* Yelp reviews. [UL-160]
* ~~Notepad import (AI Parsing) [UL-161]~~ (services/ai/document_import.py)
* XLS import [UL-162]
...
* ~~"View my profile as..." feature to help explain to user's how privacy settings are being applied. [UL-165]~~ (ProfilePreviewStart/StopView + middleware)
...
* ~~Pre-populate example tags for new users (to help explain usage of site). "Notable", "Graffiti", "Photography", "Dangerous", "Popular" [UL-173]~~ (create_default_tags on Profile create)
...
* ~~When creating new badges during pin import, allow an AI to select an emoji and color for it. [UL-176]~~ (badge style suggestions)
...
* ~~Bulk editing pins (based on search, badges, etc). For instance: Bulk set rating. [UL-193]~~ (pin_bulk.py PinBulkEditView)
* Main Map: When searching, show loading overlay [UL-194]
* ~~Main map: Way to show pins as a list (particularly when searching). [UL-196]~~ (_pin_list_panel.html)
...
* ~~Support importing kmz [UL-204]~~ (takeout_kml_to_dict handles KML/KMZ)
...
* ~~Add FAQ to about page. [UL-217]~~ (dedicated /faq/ page)
...
* ~~New Site Tool -> Delete My Data: Fully removes all the user's submitted data from everywhere on the site. [UL-220]~~ (account deletion request/cancel flow)
...
* ~~On pin details page, when street view is unavailable, hide the section. [UL-225]~~ (streetview_check metadata endpoint)
...
* ~~Implement a few extra "undos". For instance: pin deletion, etc. [UL-234]~~ (undo framework: pin/wiki/checkin/trip + Undo History page)
...
* ~~Only check US-centric APIs (like loopnet, NPS, etc) when the location is in the USA. [UL-284]~~ (USA geo-filter in rate-limited gateways)
* Max zoom out on the map still isn't quite right. Try clamping? [UL-285]
* ~~Undo pin deletion feature. (Initially mark the pin as deleted, but only realize the deletion in X days. User can undo the action in a new page.) [UL-286]~~ (cache-backed undo stash + Settings > Undo History)
...
* ~~Community wiki: Average Danger/Rating/Vulnerability scores when 5+ people have pinned it. [UL-290]~~ (WikiStatVote community votes + fuzzed counts)
* ~~Reorganize api services [UL-291]~~ (services/apis/ hierarchy; legacy services/google + smithsonian.py removed)
...
* ~~Visit log can include "with another user", which logs it / visually shows it, and notifies the other user to optionally accept the visit log as well. [UL-298]~~ (visit participants + ExternalVisitParticipant)
...
* ~~Allow multiple email addresses to make it easier for other users to find you. [UL-304]~~ (ProfileEmail secondary emails w/ verification)
...
* ~~User option to opt-out of visit entry tracking. [UL-307]~~ (track_pin_visits/track_routes/track_geolocation settings)
...
* ~~"I didn't come home" feature can be implemented via email prior to mobile app. [UL-311]~~ (safety check-ins with contact emails/portal/chat)
...
* ~~"Memories" page, showing location history and location visits over time. [UL-314]~~ (Memories feed/on-this-day/visits/photos/maps)
...
## APIs to consider
* ~~Wayback [UL-315]~~ (apis/locations/wayback_machine.py)
* ~~Apple Maps [UL-316]~~ (apis/locations/apple_maps.py)
* ~~OpenHistoricalMap [UL-317]~~ (apis/locations/openhistoricalmap.py)
...
* ~~USGS M2M / EarthExplorer [UL-319]~~ (apis/locations/usgs.py)
* ~~USGS TNM API / topoView / HTMC [UL-320]~~ (apis/locations/usgs.py)
* ~~Esri World Imagery Wayback [UL-321]~~ (apis/locations/esri.py)
* ~~OpenAerialMap [UL-322]~~ (apis/locations/open_aerial_map.py)
...
### Community
* ~~Ability to @ friends in comments, etc. [UL-330]~~ (services/mentions.py)
...
### Misc
* ~~User settings: Allow friend requests checkbox is missing. Should have additional option for "from users with one pin in common", "1 mutual", etc. [UL-346]~~ (friend_request_visibility w/ in-common options)
* ~~Clicking on notification should take you to the relevant page. [UL-347]~~ (notification items link to n.url)
