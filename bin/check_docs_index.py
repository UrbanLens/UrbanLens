...
Five invariants, all cheap and all unambiguous:
...
5. The "Next free id" header is actually free - one past the highest allocated.
...
def main() -> int:
    """Report every way the index and its entries disagree."""
    ...
    problems = _HEADING.findall(problems_path.read_text(encoding="utf-8")) if problems_path.is_file() else []
